...
var miscTranslations = {
	...
	"audio-processing-disabled":
		"Audio processing is disabled with this guest. Can't mute or change volume",
	"not-the-director":
		"<font color='red'>You are not the director of this room. You will have limited to no control. See <a target='_blank' href='https://docs.vdo.ninja/director-settings/codirector'>&codirector</a> on how to become a co-director.</font>",
	"room-is-claimed":
		"The room is already claimed by someone else.\n\nOnly the first person to join a room is the assigned director.\n\nRefresh after the first director leaves to claim.",
	...
	"room-test-not-good":
		"The room name 'test' is very commonly used and may not be secure.\n\nAre you sure you wish to proceed?",
	"load-previous-session":
		"Would you like to load your previous session's settings?",
	"enter-password":
	...
	"no-streamID-provided":
		"No streamID was provided; one will be generated randomily.\n\nStream ID: ",
	"alphanumeric-only":
		"Info: Only AlphaNumeric characters should be used for the stream ID.\n\nThe offending characters have been replaced by an underscore",
	"stream-id-too-long":
		"The Stream ID should be less than 45 alPhaNuMeric characters long.\n\nWe will trim it to length.",
	...
	transfer: "transfer",
	armed: "armed",
	"transfer-guest-to-room":
		"Transfer guests to room:\n\n(Please note: rooms must share the same password)",
	"transfer-guest-to-url":
		"Transfer guests to new website URL.\n\n(Guests will be prompted to accept)",
	...
	"confirm-disconnect-users":
		"Are you sure you wish to disconnect these users?",
	...
	"signal-meter":
		"Video packet loss indicator of video preview; green is good, red is bad. Flame implies CPU is overloaded. May not reflect the packet loss seen by scenes or other guests.",
	"waiting-for-the-stream":
		"Waiting for the stream. Tip: Adding &cleanoutput to the URL will hide this spinner, or click to retry, which will also hide it.",
	...
	"you-are-no-longer-a-co-director":
		"You are no longer a co-director as you were transferred.",
	transferred: "Transferred",
	"room-changed": "Your room has changed",
	"headphones-tip": "<i>Tip:</i> Use headphones to avoid audio echo issues.",
	"camera-tip-c922":
		"<i>Tip:</i> To achieve 60-fps with a C922 webcam, low-light compensation needs to be turned off, exposure set to auto, and 720p used.",
	"camera-tip-camlink":
		"<i>Tip:</i> A Cam Link may glitch green/purple if accessed elsewhere while already in use.",
	"samsung-a-series":
		"Samsung A-series phones may have issues with Chrome; if so, try Firefox Mobile instead or switch video codecs.",
};
...
function isOperaGX() {
	return (
		...
		!!window.opera ||
		navigator.userAgent.indexOf(" OPR/75") >= 0
	);
}
...
function deleteOldMedia() {
	...
	while (i--) {
		...
		if (videoElements[i].isConnected === false) {
			if (
				...
				(videoElements[i].srcObject &&
					videoElements[i].srcObject.active === false)
			) {
	...
	while (i--) {
		if (audioElements[i].isConnected === false) {
			if (
				audioElements[i].srcObject == null ||
				(audioElements[i].srcObject &&
					audioElements[i].srcObject.active === false)
			) {
...
async function promptAlt(
	...
	await new Promise((resolve, reject) => {
		...
		modalTemplate = `<div id="modal_${promptID}" class="promptModal" style="z-index:${
			zindex + 2
		}">	
				<div class="promptModalInner">
			...
			<div id="modalBackdrop_${promptID}" class="${backdropClass}" style="z-index:${
			zindex + 1
		}"></div>`;
		...
		document
			.getElementById("input_" + promptID)
		...
		document
			.getElementById("submit_" + promptID)
		...
		document
			.getElementById("cancel_" + promptID)
			.addEventListener("click", function (event) {
				var pid = event.target.dataset.pid;
				document.getElementById("modal_" + pid).remove();
				document.getElementById("modalBackdrop_" + pid).remove();
		...
		document
			.getElementById("close_" + promptID)
			.addEventListener("click", function (event) {
				var pid = event.target.dataset.pid;
				document.getElementById("modal_" + pid).remove();
				document.getElementById("modalBackdrop_" + pid).remove();
...
async function promptTransfer(value = null, bcmode = null, updateurl = null) {
	...
	await new Promise((resolve, reject) => {
		...
		var inputText =
			"<font style='font-size:1.2em'>" +
			miscTranslations["transfer-guest-to-room"].replace(
				"\n",
				"</font><br /><font>"
			) +
			"</font>";
...
function youveBeenTransferred() {
	getChatMessage(
		miscTranslations["you-have-been-transferred"],
		(label = false),
		(director = false),
		(overlay = true)
	); // "you-have-been-transferred"
	getById("head2").innerHTML =
		'<span data-translate="transferred-to-room">' +
		miscTranslations["room-changed"] +
		"</span>"; //
	if (session.director) {
		getById("head4").innerHTML =
			miscTranslations["you-are-no-longer-a-co-director"]; //"You are no longer a co-director as you were transferred."; //
	}
...
function warnUser(message, timeout = false) {
	...
	modalTemplate = `<div class="alertModal" id="alertModal"  style="z-index:${
		zindex + 2
	}">	
	...
	if (result1) {
		if (result2) {
			target.innerHTML =
				...
				miscTranslations["share-with-trusted"] +
				"</font>";
		} else {
			target.innerHTML =
				"<font style='color:#e67202;'>" +
				miscTranslations["pass-recommended"] +
				"</font>";
		}
	} else {
		target.innerHTML =
			"<font style='color:red'>" +
			miscTranslations["insecure-room-name"] +
			"</font> " +
			miscTranslations["allowed-chars"] +
			": <i>A-Z, a-z, 0-9, _</i>";
	}
};
...
var sanitizeRoomName = function (roomid) {
	...
	if (roomid.replace(/ /g, "_") !== sanitized) {
		if (!session.cleanOutput) {
			warnUser(
				"Info: Only AlphaNumeric characters should be used for the room name.\n\nThe offending characters have been replaced by an underscore"
			);
		}
	}
	...
	if (session.cleanOutput === false) {
		if (document.getElementById("videosource")) {
			...
			for (var uid in session.pcs) {
				if (
					session.pcs[uid].obsSourceActive !== false &&
					session.pcs[uid].obsVisibility &&
					session.pcs[uid].sceneDisplay !== false
				) {
					visibility = true;
				} else if (
					session.pcs[uid].obsVisibility &&
				...
				if (
					(session.pcs[uid].obsRecording || session.pcs[uid].obsStreaming) &&
					session.pcs[uid].obsSourceActive !== false &&
					session.pcs[uid].obsVisibility &&
					session.pcs[uid].sceneDisplay !== false
				) {
			...
			if (recording) {
				...
				let selectedChannel = window.location.search
					.split("?room=", 2)[1]
					.split("&", 1);
				let [channel] = selectedChannel;
				getById("obsState").classList.remove("recording");
				getById("obsState").classList.remove("ondeck");
				getById("obsState").innerHTML = "LIVE " + channel.toUpperCase();
			}
			...
			if (visibility) {
				...
				pulse.classList.add("badge"); // LIVE
				getById("obsState").appendChild(pulse); // LIVE
			} else {
...
function makeDraggableElement(elmnt, absolute = false) {
	...
	function dragMouseDown(e) {
		...
		document.onmouseup = function (event) {
			closeDragElement(event, elmnt);
		};

		document.onmousemove = function (event) {
			elementDrag(elmnt, event);
		};

		if ("stopDragTimeout" in elmnt) {
		...
		elmnt.onmouseleave = function (event) {
			...
			//	console.log(event);
			elmnt.stopDragTimeout = setTimeout(
				function (ele, evt1) {
				...
				if (!session.permaid) {
					if (!session.queue) {
						...
						if (
							...
							!session.exclude.includes(streamid)
						) {
							if (UUID) {
...
function setupIncomingScreenTracking(v, UUID) {
	...
	if (session.director) {
		...
		v.title =
			"Hold CTRL or CMD (⌘) while clicking the video to open detailed stats";
		if (session.beepToNotify) {
			playtone();
		}
	...
	if (session.director) {
		...
		if (session.customWSS && "isScene" in msg && msg.isScene !== false) {
			...
			createControlBoxScreenshare(
				UUID,
				"https://" +
					location.host +
					location.pathname +
					"?view=" +
					session.rpcs[UUID].streamID +
					":s&scene" +
					codecGroupFlag +
					"&room=" +
					session.roomid +
...
function setupIncomingVideoTracking(v, UUID) {
	...
	if (session.director) {
		...
		if (session.view) {
			...
			session.autoadd &&
			session.rpcs[UUID].streamID &&
			session.autoadd.includes(session.rpcs[UUID].streamID)
		) {
			/// session.autoadd
	...
	if (session.audioGain !== false) {
		if (update) {
			if (session.roomid) {
				...
				generateHash(
					session.streamID + session.roomid + pswd + session.salt,
					6
				).then(function (hash) {
		...
		generateHash(
			...
			if (volume !== "") {
				...
				for (var waid in session.webAudios) {
					...
					log(
						"Adjusting Gain; only track 0 in all likely hood, unless more than track 0 support is added."
					);
					session.webAudios[waid].gainNode.gain.setValueAtTime(
						vol,
						session.webAudios[waid].audioContext.currentTime
					);
				}
			}
		});
...
function updateMixerRun(e = false, programa) {
	...
	try {
		if (getById("subControlButtons").dragElement) {
			if (parseInt(getById("subControlButtons").style.top) > 0) {
				...
				parseInt(getById("subControlButtons").style.top) <
				parseInt(50 - window.innerHeight)
			) {
				getById("subControlButtons").style.top =
					parseInt(50 - window.innerHeight) + "px";
			}
			if (parseInt(getById("subControlButtons").style.left) < 0) {
				getById("subControlButtons").style.left = "0px";
			} else if (
				parseInt(getById("subControlButtons").style.left) >
				parseInt(window.innerWidth - getById("subControlButtons").offsetWidth)
			) {
				getById("subControlButtons").style.left =
					parseInt(
						window.innerWidth - getById("subControlButtons").offsetWidth
					) + "px";
			}
		}
		if (session.director) {
		...
		if (window.innerHeight <= 700) {
			if (document.getElementById("controlButtons")) {
				var h =
					window.innerHeight -
					hi -
					document.getElementById("controlButtons").offsetHeight;
			} else {
				var h = window.innerHeight - hi;
			}
		}
		...
		if (session.infocus === true) {
			...
			session.infocus &&
			session.infocus !== true &&
			session.infocus in session.rpcs
		) {
			// if the infocus stream is connected
			if (session.group.length) {
				try {
					if (
						session.group.some((item) =>
							session.rpcs[session.infocus].group.includes(item)
						)
					) {
		...
		if (
			...
			(session.videoElement.src || session.videoElement.srcObject)
		) {
			// I, myself, exist
			if (session.videoElement.style.display != "none") {
				...
				if (session.minipreview && soloVideo !== true) {
					...
					if (session.activeSpeaker && !session.activelySpeaking) {
						...
						session.videoElement.srcObject &&
						session.videoElement.srcObject.getVideoTracks().length === 0
					) {
						// do not show a video element if its completely empty.
		...
		if (soloVideo && soloVideo in session.rpcs) {
			...
			for (var j in session.rpcs) {
				if (session.group.length) {
					try {
						if (
							!session.group.some((item) =>
								session.rpcs[j].group.includes(item)
							)
						) {
							continue;
						}
				...
				if (j != soloVideo) {
					...
					try {
						if (
							...
							session.rpcs[j].videoElement.style.display != "none"
						) {
							...
							if (
								...
								document.pictureInPictureElement.id ==
									session.rpcs[j].videoElement.id
							) {
							...
							if (
								session.rpcs[j].videoElement.srcObject &&
								session.rpcs[j].videoElement.srcObject.getAudioTracks().length
							) {
					...
					try {
						if (session.rpcs[j].iframeEle) {
							...
							if (
								session.rpcs[j].targetBandwidth !== -1 &&
								session.rpcs[j].targetBandwidth < session.zoomedBitrate
							) {
			...
			for (var i in session.rpcs) {
				...
				if (session.group.length) {
					try {
						if (
							!session.group.some((item) =>
								session.rpcs[i].group.includes(item)
							)
						) {
							continue;
						}
				...
				if (session.rpcs[i].videoElement) {
					...
					if (session.rpcs[i].videoElement.style.display != "none") {
						if (
							session.rpcs[i].videoElement.srcObject &&
							session.rpcs[i].videoElement.srcObject.getVideoTracks().length
						) {
			...
			if (
				...
				totalRoomBitrate = Math.min(
					session.controlRoomBitrate,
					totalRoomBitrate
				);
			}
			...
			if (session.screenShareBitrate !== false) {
				...
				if (roomQuality - screenShareTotal > 0) {
					roomBitrate = parseInt(
						totalRoomBitrate / (roomQuality - screenShareTotal)
					);
					if (session.totalSceneBitrate) {
						sceneBitrate = parseInt(
							session.totalSceneBitrate / (roomQuality - screenShareTotal)
						);
				...
				try {
					if (session.roomid !== false && session.scene === false) {
						if (roomQuality - screenShareTotal <= 0) {
							...
							roomBitrate = parseInt(
								(totalRoomBitrate - screenShareBitrate) /
									(roomQuality - screenShareTotal)
							);
						}
						...
						if (roomQuality - screenShareTotal <= 0) {
							...
							screenShareBitrate = parseInt(
								totalRoomBitrate / (1.5 * screenShareTotal)
							);
							sceneBitrate = parseInt(
								(totalRoomBitrate - screenShareBitrate) /
									(roomQuality - screenShareTotal)
			...
			for (var keyIndex = 0; keyIndex < RPCSkeys.length; keyIndex++) {
						...
						if (
							...
							if (session.scene !== false) {
								if (session.groupAudio) {
									session.requestRateLimit(
										session.hiddenSceneViewBitrate,
										i,
										false
									);
								} else {
									session.requestRateLimit(
										...
										i,
										true
									); // hidden. I dont want it to be super low, for video quality reasons.
				...
				if (session.rpcs[i].imageElement) {
					if (
						session.rpcs[i].videoElement &&
						session.rpcs[i].videoElement.srcObject.getAudioTracks().length
					) {
						// is there audio?
					...
					if (
						session.rpcs[i].videoMuted ||
						session.rpcs[i].directorVideoMuted ||
						session.rpcs[i].virtualHangup ||
						session.rpcs[i].bandwidthMuted
					) {
						continue;
					}
				...
				if (session.rpcs[i].videoElement) {
					...
					if (session.rpcs[i].videoElement.style.display == "none") {
						...
						if (sceneBitrate !== false) {
							if (
								screenShareBitrate !== false &&
								session.rpcs[i].screenShareState
							) {
		...
		if (document.fullscreenElement) {
			...
			for (var i = 0; i < mediaPool.length; i++) {
				if (mediaPool[i].id == document.fullscreenElement.id) {
					// make sure the element is suppose to be seen.
				...
				if (mediaPool[NW - 1]) {
					...
					if (mediaPool[NW - 1].dataset.UUID) {
						if (mediaPool[NW - 1].dataset.UUID in session.rpcs) {
							if (
								session.rpcs[mediaPool[NW - 1].dataset.UUID].screenShareState
							) {
								sscount += 1;
						...
						"id" in mediaPool[NW - 1] &&
						mediaPool[NW - 1].id == "screensharesource"
					) {
						sscount += 1;
						sssid = mediaPool[NW - 1].dataset.sid;
					}
		...
		if (playarea.querySelector("#guestFeeds")) {
			...
			for (var n = 0; n < childNodes.length; n++) {
				if (childNodes[n].querySelector("video")) {
					...
					for (var m = 0; m < mediaPool.length; m++) {
						if (
							mediaPool[m].tagName.toLowerCase() === "iframe" &&
							mediaPool[m].src &&
							iftemp.src === mediaPool[m].src
						) {
	...
	if (
		...
		if ("playlist" in session.videoElement) {
			...
			if (
				session.videoElement.srcObject &&
				session.videoElement.srcObject.getVideoTracks().length
			) {
					...
					if (mpl === 0 && session.minipreview === 2) {
						...
						if (soloVideo !== true) {
							...
							togglePreview.onclick = function(event){
								event.preventDefault();
								event.stopPropagation();
								container.querySelector("video").classList.toggle("advanced");
								return false;
							};
						...
						if (document.getElementById("minipreview")) {
							...
							try {
								...
								container.style.maxHeight =
									parseInt(getById("gridlayout").offsetHeight) + "px";
								togglePreview.style.top = "calc(" + hi + "px + 2vh)";
								togglePreview.style.maxHeight =
									parseInt(getById("gridlayout").offsetHeight) + "px";
							} catch (e) {
							...
							togglePreview.innerHTML =
								'<i class="las la la-eye-slash"></i><i class="las la la-eye"></i>';
	...
	try {
		...
		if (session.slots) {
			...
			while (j--) {
				...
				if (
					!("slot" in mediaPool[j]) ||
					!parseInt(mediaPool[j].slot) ||
					mediaPool[j].slot == "0" ||
					!mediaPool[j].slot ||
					session.slots < parseInt(mediaPool[j].slot)
				) {
		...
		try {
			...
			if (i !== 0) {
				if (Math.ceil((i + 0.01) / rw) == rh) {
					if (mpl % rw) {
						offsetx = Math.max(
							((rw - (mpl % rw)) * (window.innerWidth / rw)) / 2,
							0
						);
					}
				}
			...
			if (vid.alreadyAdded && vid.alreadyAdded == true) {
				...
				if (session.animatedMoves) {
					if (typeof session.layout === "object" && session.layout !== null) {
						if (vid.dataset.sid in session.layout) {
							var left =
								(window.innerWidth / 100) * session.layout[vid.dataset.sid].x ||
								0;
							var top =
								(window.innerHeight / 100) *
									session.layout[vid.dataset.sid].y || 0;
							var width =
								(window.innerWidth / 100) * session.layout[vid.dataset.sid].w ||
								0;
							var height =
								(window.innerHeight / 100) *
									session.layout[vid.dataset.sid].h || 0;
							...
							session.requestRateLimit(
								...
								i,
								false
							); // it's added already, so we know it needs sound.  But lets d
							vid.style.width = "0px";
							vid.style.height = "0px";
							vid.style.top = "0px";
							vid.style.left = "0px";
						...
						if (vid.dataset.sid in customLayout) {
							var left =
								(window.innerWidth / 100) * customLayout[vid.dataset.sid].x ||
								0;
							var top =
								(window.innerHeight / 100) * customLayout[vid.dataset.sid].y ||
								0;
							var width =
								(window.innerWidth / 100) * customLayout[vid.dataset.sid].w ||
								0;
							var height =
								(window.innerHeight / 100) * customLayout[vid.dataset.sid].h ||
								0;
						...
						container.style.zIndex = 0;
						var left = Math.max(
							offsetx + Math.floor((((i % rw) + 0) * w) / rw),
							0
						);
						var top = Math.max(
							offsety + Math.floor(((Math.floor(i / rw) + 0) * h) / rh + hi),
							0
						);
						...
						function (CCC) {
							try {
								...
								if (ww <= 1 && ww >= -1) {
									...
									skip = false;
									CCC.style.width =
										parseInt(parseInt(CCC.style.width) - ww / 1.5) + "px";
								}
								...
								if (hh <= 1 && hh >= -1) {
									...
									skip = false;
									CCC.style.height =
										parseInt(parseInt(CCC.style.height) - hh / 1.5) + "px";
								}
								...
								if (tt <= 1 && tt >= -1) {
									...
									skip = false;
									CCC.style.top =
										parseInt(parseInt(CCC.style.top) - tt / 1.5) + "px";
								}
								...
								if (ll <= 1 && ll >= -1) {
									...
									skip = false;
									CCC.style.left =
										parseInt(parseInt(CCC.style.left) - ll / 1.5) + "px";
								}
					...
					container.style.left =
						offsetx + Math.floor((((i % rw) + 0) * w) / rw) + "px";
					container.style.top =
						offsety +
						Math.floor(((Math.floor(i / rw) + 0) * h) / rh + hi) +
						"px";
					container.style.width = Math.ceil(w / rw) + "px";
					container.style.height = Math.ceil(h / rh) + "px";
				}
			} else if (
				typeof session.layout === "object" &&
				session.layout !== null
			) {
				//////////////////
				...
				if (vid.dataset.sid in session.layout) {
					...
					var left =
						(window.innerWidth / 100) * session.layout[vid.dataset.sid].x || 0;
					var top =
						(window.innerHeight / 100) * session.layout[vid.dataset.sid].y || 0;
					var width =
						(window.innerWidth / 100) * session.layout[vid.dataset.sid].w || 0;
					var height =
						(window.innerHeight / 100) * session.layout[vid.dataset.sid].h || 0;
				...
				if (vid.dataset.sid in customLayout) {
					...
					var left =
						(window.innerWidth / 100) * customLayout[vid.dataset.sid].x || 0;
					var top =
						(window.innerHeight / 100) * customLayout[vid.dataset.sid].y || 0;
					var width =
						(window.innerWidth / 100) * customLayout[vid.dataset.sid].w || 0;
					var height =
						(window.innerHeight / 100) * customLayout[vid.dataset.sid].h || 0;
				...
				container.style.left =
					offsetx + Math.floor((((i % rw) + 0) * w) / rw) + "px";
				container.style.top =
					offsety + Math.floor(((Math.floor(i / rw) + 0) * h) / rh + hi) + "px";
				container.style.width = Math.ceil(w / rw) + "px";
				container.style.height = Math.ceil(h / rh) + "px";
					...
					if (vid.dataset.UUID) {
						if (wrw && hrh) {
							if (session.devicePixelRatio) {
								session.requestResolution(
									vid.dataset.UUID,
									wrw * session.devicePixelRatio,
									hrh * session.devicePixelRatio
								);
							} else if (
								...
								session.requestResolution(
									vid.dataset.UUID,
									wrw * window.devicePixelRatio,
									hrh * window.devicePixelRatio
								);
							} else {
			...
			if (session.cover) {
				...
				if ("rotated" in vid && (vid.rotated == 90 || vid.rotated == 270)) {
					...
					if (customSet) {
						if (asw < ash) {
							...
							holder.style.left = Math.ceil(
								(width - Math.ceil(vid.videoHeight * asw)) / 2
							);
							holder.style.top = Math.ceil(
								(height - Math.ceil(vid.videoWidth * asw)) / 2
							);
						} else {
							holder.style.width = Math.ceil(vid.videoHeight * ash) + "px";
							holder.style.height = Math.ceil(vid.videoWidth * ash) + "px";
							holder.style.left = Math.ceil(
								(width - Math.ceil(vid.videoHeight * ash)) / 2
							);
							holder.style.top = Math.ceil(
								(height - Math.ceil(vid.videoWidth * ash)) / 2
							);
						}
					} else if (asw < ash) {
						holder.style.width = Math.ceil(vid.videoHeight * asw) + "px";
						holder.style.height = Math.ceil(vid.videoWidth * asw) + "px";
						holder.style.left = Math.ceil(
							(Math.ceil(w / rw) - Math.ceil(vid.videoHeight * asw)) / 2
						);
						holder.style.top = Math.ceil(
							(Math.ceil(h / rh) - Math.ceil(vid.videoWidth * asw)) / 2
						);
					} else {
						holder.style.width = Math.ceil(vid.videoHeight * ash) + "px";
						holder.style.height = Math.ceil(vid.videoWidth * ash) + "px";
						holder.style.left = Math.ceil(
							(Math.ceil(w / rw) - Math.ceil(vid.videoHeight * ash)) / 2
						);
						holder.style.top = Math.ceil(
							(Math.ceil(h / rh) - Math.ceil(vid.videoWidth * ash)) / 2
						);
					}
					...
					if (ChromeVersion && ChromeVersion < 77) {
						if (
							!session.animatedMoves &&
							parseInt(container.style.width) > parseInt(holder.style.height)
						) {
							vid.style.position = "relative";
							...
							errorlog(
								parseInt(container.style.width) +
									" " +
									parseInt(holder.style.height)
							);
						} else if (
							session.animatedMoves &&
							container.twidth &&
							parseInt(container.twidth) > parseInt(holder.style.height)
						) {
							errorlog(
								container.twidth &&
									parseInt(container.twidth) +
										" " +
										parseInt(holder.style.height)
							);
							vid.style.position = "relative";
							vid.style.objectFit = "contain"; //contain;
							vid.style.left =
								(parseFloat(container.twidth) - parseFloat(vid.style.width)) /
									2 +
								"px";
						} else {
							vid.style.left = 0;
						...
						vid.style.position = "relative";
						vid.style.left =
							parseFloat(holder.style.left) -
							(parseFloat(holder.style.height) -
								parseFloat(holder.style.width)) /
								2 +
							"px";
					}
					//}
					...
					if (customSet) {
						if (asw < ash) {
							...
							holder.style.left = Math.ceil(
								(width - Math.ceil(vid.videoWidth * asw)) / 2
							);
							holder.style.top = Math.ceil(
								(height - Math.ceil(vid.videoHeight * asw)) / 2
							);
						} else {
							holder.style.width = Math.ceil(vid.videoWidth * ash) + "px";
							holder.style.height = Math.ceil(vid.videoHeight * ash) + "px";
							holder.style.left = Math.ceil(
								(width - Math.ceil(vid.videoWidth * ash)) / 2
							);
							holder.style.top = Math.ceil(
								(height - Math.ceil(vid.videoHeight * ash)) / 2
							);
						}
					} else if (asw < ash) {
						holder.style.width = Math.ceil(vid.videoWidth * asw) + "px";
						holder.style.height = Math.ceil(vid.videoHeight * asw) + "px";
						holder.style.left = Math.ceil(
							(Math.ceil(w / rw) - Math.ceil(vid.videoWidth * asw)) / 2
						);
						holder.style.top = Math.ceil(
							(Math.ceil(h / rh) - Math.ceil(vid.videoHeight * asw)) / 2
						);
					} else {
						holder.style.width = Math.ceil(vid.videoWidth * ash) + "px";
						holder.style.height = Math.ceil(vid.videoHeight * ash) + "px";
						holder.style.left = Math.ceil(
							(Math.ceil(w / rw) - Math.ceil(vid.videoWidth * ash)) / 2
						);
						holder.style.top = Math.ceil(
							(Math.ceil(h / rh) - Math.ceil(vid.videoHeight * ash)) / 2
						);
					}
				} else {
					var asw = wrw / vid.videoWidth;
				...
				if (customSet) {
					if (asw < ash) {
						...
						holder.style.left = Math.ceil(
							(width - Math.ceil(vid.width * asw)) / 2
						);
						holder.style.top = Math.ceil(
							(height - Math.ceil(vid.height * asw)) / 2
						);
					} else {
						holder.style.width = Math.ceil(vid.width * ash) + "px";
						holder.style.height = Math.ceil(vid.height * ash) + "px";
						holder.style.left = Math.ceil(
							(width - Math.ceil(vid.width * ash)) / 2
						);
						holder.style.top = Math.ceil(
							(height - Math.ceil(vid.height * ash)) / 2
						);
					}
				} else if (asw < ash) {
					holder.style.width = Math.ceil(vid.width * asw) + "px";
					holder.style.height = Math.ceil(vid.height * asw) + "px";
					holder.style.left = Math.ceil(
						(Math.ceil(w / rw) - Math.ceil(vid.width * asw)) / 2
					);
					holder.style.top = Math.ceil(
						(Math.ceil(h / rh) - Math.ceil(vid.height * asw)) / 2
					);
				} else {
					holder.style.width = Math.ceil(vid.width * ash) + "px";
					holder.style.height = Math.ceil(vid.height * ash) + "px";
					holder.style.left = Math.ceil(
						(Math.ceil(w / rw) - Math.ceil(vid.width * ash)) / 2
					);
					holder.style.top = Math.ceil(
						(Math.ceil(h / rh) - Math.ceil(vid.height * ash)) / 2
					);
				}
			...
			if (
				...
				if (selectedProgram == "hoymismo") {
					...
					watermark.src = "./media/DP.png";
				} else if (selectedProgram == "tn5E") {
					watermark.src = "./media/WATERMARKS-TN5.png";
				} else if (selectedProgram == "tn5M") {
					watermark.src = "./media/WATERMARKS-TN5.png";
				} else if (selectedProgram == "latarde") {
					watermark.src = "./media/WATERMARKS-TSI.png";
				} else {
					watermark.src = null;
				}
			...
			if (session.signalMeter) {
				if (vid.dataset.UUID && !session.rpcs[vid.dataset.UUID].signalMeter) {
					session.rpcs[vid.dataset.UUID].signalMeter = getById(
						"signalMeterTemplate"
					).cloneNode(true);
					session.rpcs[vid.dataset.UUID].signalMeter.classList.remove(
						"advanced"
					);
					session.rpcs[vid.dataset.UUID].signalMeter.id =
						"signalMeter_" + vid.dataset.UUID;
					session.rpcs[vid.dataset.UUID].signalMeter.dataset.level = 0;
					session.rpcs[vid.dataset.UUID].signalMeter.title =
						miscTranslations["signal-meter"];
					holder.appendChild(session.rpcs[vid.dataset.UUID].signalMeter);
				} else if (
					vid.dataset.UUID &&
					session.rpcs[vid.dataset.UUID].signalMeter
				) {
					holder.appendChild(session.rpcs[vid.dataset.UUID].signalMeter);
				}
			}
			...
			if (session.ruleOfThirds) {
				if (vid.id == "videosource") {
					...
					svg.innerHTML =
						'<svg viewBox="0 0 800 600"  preserveAspectRatio="none" style="width:100%;height:100%;"><g>\
					\
						...
						if (vid.tagName.toLowerCase() == "video") {
							...
							if (playPromise !== undefined) {
								playPromise
									...
									.catch((err) => {
										var bigPlayButton =
											document.getElementById("bigPlayButton");
										if (bigPlayButton) {
			...
			if (!session.cleanOutput && !session.nocursor) {
				if (session.roomid !== false && session.scene === false) {
					if (!(vid.id === "videosource" && session.minipreview)) {
						...
						if (soloVideo) {
							button.innerHTML =
								"<img src='./media/sd.svg' style='background-color:#0007;width:4vh' aria-hidden='true' />";
							button.title = "Show all active videos togethers";
						} else if (mpl > 1) {
							button.innerHTML =
								"<img src='./media/hd.svg' style='background-color:#0007;width:4vh' aria-hidden='true' />";
...
/* document
	.getElementById("loadButtonWatermark")
	.addEventListener("click", function () {
		let optionWaterMarker = document.getElementById("selectWaterMarker").value;
		updateMixerRun(true, optionWaterMarker);
	}); Jorge */
				...
				response.json().then(function (data) {
					...
					if (translationBacklog.length) {
						for (var i = 0; i < translationBacklog.length; i++) {
							try {
								miniTranslate(
									translationBacklog[i][0],
									translationBacklog[i][1]
								);
							} catch (e) {}
						}
						translationBacklog = [];
			...
			if (e.data.action == "screen-share-ended") {
				if (session.screenShareElement) {
					if (e.source == session.screenShareElement.contentWindow) {
						...
						session.screenShareElement.parentNode.removeChild(
...
async function jumptoroom(event = null) {
	...
	if (roomname.length) {
		var passStr = "";
		window.focus();
		var pass = await promptAlt(
			"Enter a password if provided, otherwise just click Cancel",
			false,
			true
		); //sanitizePassword(session.password);
		if (pass && pass.length) {
...
function setupCanvas() {
	...
	if (session.canvas === null) {
		...
		if (iOS || iPad) {
			...
			session.canvasSource.controls = session.showControls;
...
function applyEffects(track) {
	// video only please. do not touch audio.  Run updateRenderOutpipe() instead of this directly.
	log("applyEffects()");

	if (session.effects == "0" || !session.effects) {
		// auto align face
		return track;
	} else if (session.effects == "1") {
		// auto align face
		setupCanvas();
...
async function makeImages(startup = false) {
	...
	if (makeImagesActive === null) {
		...
		session.webPcanvasCtx = session.webPcanvas.getContext("2d", {
			alpha: false,
			desynchronized: true,
		});
	...
	if (session.streamSrc.getVideoTracks().length === 0) {
		...
		session.webPcanvas.makeImagesTimeout = setTimeout(function () {
			makeImages();
		}, timeout * 3);
		return;
	}
			...
			try {
				if (session.pcs[i].allowBroadcast) {
					...
					if (!session.pcs[i].sendChannel.bufferedAmount) {
						if (!arrayBuffer) {
							session.webPcanvasCtx.drawImage(
								session.videoElement,
								0,
								0,
								session.webPcanvas.width,
								session.webPcanvas.height
							);
							arrayBuffer = dataURItoArraybuffer(
								session.webPcanvas.toDataURL("image/webp", 0.6)
							);
						}
	...
	if (broadcasting) {
		...
		var time =
			...
			(session.webPcanvas.nowTime - session.webPcanvas.lastTime);
		if (time <= 0) {
			session.webPcanvas.makeImagesTimeout = setTimeout(function () {
				makeImages();
			}, 0);
		...
		try {
			...
			for (var UUID in session.rpcs) {
				if (
					...
					session.rpcs[UUID].streamSrc &&
					session.rpcs[UUID].streamSrc.getTracks().length
				) {
				...
				if (
					session.rpcs[UUID].videoMuted ||
					(!session.rpcs[UUID].imageElement && !session.rpcs[UUID].canvas) ||
					(session.infocus && session.infocus !== UUID)
				) {
					if (session.directorList.indexOf(UUID) >= 0) {
					...
					if (
						session.rpcs[UUID].remoteMuteState ||
						!session.rpcs[UUID].streamSrc
					) {
...
function TFLiteWorker() {
	...
	function process() {
		...
		try {
			...
			for (let i = 0; i < segmentationPixelCount; i++) {
				session.tfliteModule.HEAPF32[inputMemoryOffset + i * 3] =
					imageData.data[i * 4] / 255;
				session.tfliteModule.HEAPF32[inputMemoryOffset + i * 3 + 1] =
					imageData.data[i * 4 + 1] / 255;
				session.tfliteModule.HEAPF32[inputMemoryOffset + i * 3 + 2] =
			...
			for (let i = 0; i < segmentationPixelCount; i++) {
				const background =
					session.tfliteModule.HEAPF32[outputMemoryOffset + i * 2];
				const person =
					session.tfliteModule.HEAPF32[outputMemoryOffset + i * 2 + 1];
			...
			if (session.effects == "4") {
				// greenscreen
				session.canvasCtx.filter = "none";
				session.canvasCtx.fillStyle = "#0F0";
				session.canvasCtx.fillRect(
					0,
					0,
				...
				if (session.effectValue) {
					session.canvasCtx.filter =
						"blur(" + parseInt(session.effectValue) * 2 + "px)";
				} else {
		...
		session.tfliteModule.nowTime = new Date().getTime();
		var time =
			33 - (session.tfliteModule.nowTime - session.tfliteModule.lastTime);
		time = time + session.tfliteModule.offsetTime;
		session.tfliteModule.activelyProcessing = false;
		if (time <= 0) {
			session.tfliteModule.timeout = setTimeout(function () {
				process();
			}, 0);
			session.tfliteModule.offsetTime = 0;
		} else {
	...
	function processiOS() {
		...
		try {
			...
			for (let i = 0; i < segmentationPixelCount; i++) {
				...
				const personExp = Math.exp(person - shift);
				segmentationMask.data[i * 4 + 3] =
					255 - (255 * personExp) / (backgroundExp + personExp); // softmax
			}
	...
	if (iOS || iPad || SafariVersion) {
		...
		var mulTable = [
			...
			253, 245, 119, 231, 224, 109, 211, 103, 25, 195, 189, 23, 45, 175, 171,
			83, 81, 79, 155, 151, 147, 9, 141, 137, 67, 131, 129, 251, 123, 30, 235,
			115, 113, 221, 217, 53, 13, 51, 50, 49, 193, 189, 185, 91, 179, 175, 43,
			169, 83, 163, 5, 79, 155, 19, 75, 147, 145, 143, 35, 69, 17, 67, 33, 65,
			255, 251, 247, 243, 239, 59, 29, 229, 113, 111, 219, 27, 213, 105, 207,
			51, 201, 199, 49, 193, 191, 47, 93, 183, 181, 179, 11, 87, 43, 85, 167,
			165, 163, 161, 159, 157, 155, 77, 19, 75, 37, 73, 145, 143, 141, 35, 138,
			137, 135, 67, 33, 131, 129, 255, 63, 250, 247, 61, 121, 239, 237, 117, 29,
			229, 227, 225, 111, 55, 109, 216, 213, 211, 209, 207, 205, 203, 201, 199,
			197, 195, 193, 48, 190, 47, 93, 185, 183, 181, 179, 178, 176, 175, 173,
			171, 85, 21, 167, 165, 41, 163, 161, 5, 79, 157, 78, 154, 153, 19, 75,
			149, 74, 147, 73, 144, 143, 71, 141, 140, 139, 137, 17, 135, 134, 133, 66,
			131, 65, 129, 1,
		];
		...
		var shgTable = [
			...
			21, 21, 21, 20, 21, 17, 21, 21, 21, 22, 22, 21, 22, 22, 21, 22, 21, 19,
			22, 22, 19, 20, 22, 22, 21, 21, 21, 22, 22, 22, 18, 22, 22, 21, 22, 22,
			23, 22, 20, 23, 22, 22, 23, 23, 21, 19, 21, 21, 21, 23, 23, 23, 22, 23,
			23, 21, 23, 22, 23, 18, 22, 23, 20, 22, 23, 23, 23, 21, 22, 20, 22, 21,
			22, 24, 24, 24, 24, 24, 22, 21, 24, 23, 23, 24, 21, 24, 23, 24, 22, 24,
			24, 22, 24, 24, 22, 23, 24, 24, 24, 20, 23, 22, 23, 24, 24, 24, 24, 24,
			24, 24, 23, 21, 23, 22, 23, 24, 24, 24, 22, 24, 24, 24, 23, 22, 24, 24,
			25, 23, 25, 25, 23, 24, 25, 25, 24, 22, 25, 25, 25, 24, 23, 24, 25, 25,
			25, 25, 25, 25, 25, 25, 25, 25, 25, 25, 23, 25, 23, 24, 25, 25, 25, 25,
			25, 25, 25, 25, 25, 24, 22, 25, 25, 23, 25, 25, 20, 24, 25, 24, 25, 25,
			22, 24, 25, 24, 25, 24, 25, 25, 24, 25, 25, 25, 25, 22, 25, 25, 25, 24,
			25, 24, 25, 18,
		];
		var shgSum = shgTable[amount];

		log(
			"session.canvas: " + session.canvas.width + "x" + session.canvas.height
		);
			...
			if (isIFrame) {
				...
				for (var i in session.pcs) {
					if (!session.pcs[i].sendChannel.bufferedAmount) {
						...
						session.sendMessage(
							{ effectsData: output, eID: session.effects },
							i
						);
					}
				}
...
function drawFace() {
	...
	faceAlignment = (function () {
		if (
			!(
				session.canvasSource &&
				session.canvasSource.srcObject.getVideoTracks().length
			)
		) {
			setTimeout(function () {
				drawFace();
			}, 1000); // TODO: this needs a way to prevent the function loading twice
			return;
		}
		...
		function draw() {
			...
			if (!vid.videoWidth) {
				timers.timeoutDraw = setTimeout(function () {
					draw();
				}, 1000);
				timers.activelyProcessingDraw = false;
...
function remoteStats(msg, UUID) {
		...
		for (var uuid in msg.remoteStats) {
			if (
				...
				if (msg.remoteStats[uuid].video_encoder) {
					output +=
						"<br/>video_encoder: " + msg.remoteStats[uuid].video_encoder;
				}
				...
				if ("nacks_per_second" in msg.remoteStats[uuid]) {
					output +=
						"<br/>nacks_per_second: " + msg.remoteStats[uuid].nacks_per_second;
				}
				if (msg.remoteStats[uuid].retransmitted_kbps) {
					output +=
						"<br/>retransmitted_kbps: " +
						msg.remoteStats[uuid].retransmitted_kbps;
				}
				if (msg.remoteStats[uuid].quality_limitation_reason) {
					output +=
						"<br/>quality_limitation_reason: " +
						msg.remoteStats[uuid].quality_limitation_reason;
				}
			}
		}
		warnUser(output);
	...
	if (isIFrame) {
		parent.postMessage(
			{ remoteStats: msg.remoteStats, streamID: session.rpcs[UUID].streamID },
			"*"
		);
	}
}
		...
		session.rpcs[UUID].getStats().then(function (stats) {
			...
			stats.forEach((stat) => {
				if (stat.type == "candidate-pair" && stat.nominated == true) {
					session.rpcs[UUID].stats["Peer-to-Peer Connection"]._local_ice_id =
						stat.localCandidateId;
					session.rpcs[UUID].stats["Peer-to-Peer Connection"]._remote_ice_id =
						stat.remoteCandidateId;
					session.rpcs[UUID].stats[
						"Peer-to-Peer Connection"
					].Round_Trip_Time_ms = stat.currentRoundTripTime * 1000;
					...
					if (stat.id in session.rpcs[UUID].stats) {
						...
						session.rpcs[UUID].stats[stat.id].Buffer_Delay_in_ms =
							parseInt(
								(1000 *
									(parseFloat(stat.jitterBufferDelay) -
										session.rpcs[UUID].stats[stat.id]._jitter_delay)) /
									(parseInt(stat.jitterBufferEmittedCount) -
										session.rpcs[UUID].stats[stat.id]._jitter_count)
							) || 0;
						session.rpcs[UUID].stats[stat.id]._jitter_delay =
							parseFloat(stat.jitterBufferDelay) || 0;
						session.rpcs[UUID].stats[stat.id]._jitter_count =
							parseInt(stat.jitterBufferEmittedCount) || 0;
						if ("frameWidth" in stat) {
							if ("frameHeight" in stat) {
								session.rpcs[UUID].stats[stat.id].Resolution =
									stat.frameWidth + " x " + stat.frameHeight;
								session.rpcs[UUID].stats[stat.id]._frameWidth = stat.frameWidth;
								session.rpcs[UUID].stats[stat.id]._frameHeight =
									stat.frameHeight;
							}
						}
					} else {
					...
					if (
						"_remote_ice_id" in
							session.rpcs[UUID].stats["Peer-to-Peer Connection"] &&
						session.rpcs[UUID].stats["Peer-to-Peer Connection"]
							._remote_ice_id != stat.id
					) {
						return;
					} // not matched to nominated one

					if ("candidateType" in stat) {
						session.rpcs[UUID].stats[
							"Peer-to-Peer Connection"
						].remote_candidateType = stat.candidateType;
						if (stat.candidateType === "relay") {
							if ("relayProtocol" in stat) {
								session.rpcs[UUID].stats[
									"Peer-to-Peer Connection"
								].remote_relayProtocol = stat.relayProtocol;
							}
							if ("ip" in stat) {
								session.rpcs[UUID].stats[
									"Peer-to-Peer Connection"
								].remote_relay_IP = stat.ip;
							}
						} else {
							try {
								delete session.rpcs[UUID].stats["Peer-to-Peer Connection"]
									.local_relayIP;
								delete session.rpcs[UUID].stats["Peer-to-Peer Connection"]
									.local_relayProtocol;
							} catch (e) {}
					...
					if ("networkType" in stat) {
						session.rpcs[UUID].stats[
							"Peer-to-Peer Connection"
						].remote_networkType = stat.networkType;
					}
					...
					if ("candidateType" in stat) {
						session.rpcs[UUID].stats[
							"Peer-to-Peer Connection"
						].local_candidateType = stat.candidateType;
						if (stat.candidateType === "relay") {
							if ("relayProtocol" in stat) {
								session.rpcs[UUID].stats[
									"Peer-to-Peer Connection"
								].local_relayProtocol = stat.relayProtocol;
							}
							if ("ip" in stat) {
								session.rpcs[UUID].stats[
									"Peer-to-Peer Connection"
								].local_relayIP = stat.ip;
							}
						} else {
							try {
								delete session.rpcs[UUID].stats["Peer-to-Peer Connection"]
					...
					if ("bytesReceived" in stat) {
						if (
							...
							if (
								session.rpcs[UUID].stats["Peer-to-Peer Connection"]._timestamp
							) {
								if (stat.timestamp) {
									session.rpcs[UUID].stats[
										"Peer-to-Peer Connection"
									].total_recv_bitrate_kbps = parseInt(
										(8 *
											(stat.bytesReceived -
												session.rpcs[UUID].stats["Peer-to-Peer Connection"]
													._bytesReceived)) /
									...
									hideStreamLowBandwidth(
										session.rpcs[UUID].stats["Peer-to-Peer Connection"]
											.total_recv_bitrate_kbps,
										UUID
									);
								}
							}
						}
						session.rpcs[UUID].stats["Peer-to-Peer Connection"]._bytesReceived =
							stat.bytesReceived;
					}
					if ("timestamp" in stat) {
						...
						if (
							...
							session.rpcs[UUID].stats[
								"Peer-to-Peer Connection"
							]._timestampStart = stat.timestamp;
						} else {
							session.rpcs[UUID].stats[
								"Peer-to-Peer Connection"
							].time_active_minutes =
					...
					session.rpcs[UUID].stats[stat.trackId] =
						session.rpcs[UUID].stats[stat.trackId] || {};
					session.rpcs[UUID].stats[stat.trackId].Bitrate_in_kbps = parseInt(
						(8 *
							(stat.bytesReceived -
								session.rpcs[UUID].stats[stat.trackId]._last_bytes)) /
							(stat.timestamp -
								session.rpcs[UUID].stats[stat.trackId]._last_time)
					);
					session.rpcs[UUID].stats[stat.trackId]._last_bytes =
						stat.bytesReceived ||
						session.rpcs[UUID].stats[stat.trackId]._last_bytes;
					...
					if (stat.mediaType == "video") {
						...
						if (
							session.obsfix &&
							"codec" in session.rpcs[UUID].stats &&
							session.rpcs[UUID].stats.codec == "video/VP8"
						) {
							session.rpcs[UUID].stats[stat.trackId].pliDelta =
								stat.pliCount -
									session.rpcs[UUID].stats[stat.trackId]
										.keyFramesRequested_pli || 0;
							session.rpcs[UUID].stats[stat.trackId].nackTrigger =
								stat.nackCount -
									session.rpcs[UUID].stats[stat.trackId]
										.streamErrors_nackCount +
									session.rpcs[UUID].stats[stat.trackId].nackTrigger || 0;

							log("OBS PLI FIX MODE ON");
							if (
								session.rpcs[UUID].stats[stat.trackId].pliDelta === 0 &&
								session.rpcs[UUID].stats[stat.trackId].nackTrigger >=
									session.obsfix
							) {
							...
							session.obsfix &&
							"codec" in session.rpcs[UUID].stats &&
							session.rpcs[UUID].stats.codec == "video/VP9"
						) {
							session.rpcs[UUID].stats[stat.trackId].pliDelta =
								stat.pliCount -
									session.rpcs[UUID].stats[stat.trackId]
						...
						session.rpcs[UUID].stats[stat.trackId].keyFramesRequested_pli =
							stat.pliCount || 0;
						session.rpcs[UUID].stats[stat.trackId].streamErrors_nackCount =
							stat.nackCount || 0;

						//warnlog(stat);

						if ("framesPerSecond" in stat) {
							session.rpcs[UUID].stats[stat.trackId].FPS = parseInt(
							...
							try {
								lastFramesDecoded =
									session.rpcs[UUID].stats[stat.trackId]._framesDecoded;
								lastTimestamp =
									session.rpcs[UUID].stats[stat.trackId]._timestamp;
							} catch (e) {}
							session.rpcs[UUID].stats[stat.trackId].FPS =
								parseInt(
									(10 * (stat.framesDecoded - lastFramesDecoded)) /
										(stat.timestamp / 1000 - lastTimestamp)
								) / 10;
						...
						if ("audioLevel" in stat) {
							session.rpcs[UUID].stats[stat.trackId].audio_level =
								parseInt(parseFloat(stat.audioLevel) * 10000) / 10000.0;
						}
					}

					if ("packetsLost" in stat && "packetsReceived" in stat) {
						if (!("_packetsLost" in session.rpcs[UUID].stats[stat.trackId])) {
							session.rpcs[UUID].stats[stat.trackId]._packetsLost =
								stat.packetsLost;
						}
						if (
							!("_packetsReceived" in session.rpcs[UUID].stats[stat.trackId])
						) {
							session.rpcs[UUID].stats[stat.trackId]._packetsReceived =
								stat.packetsReceived;
						}

						if (
							!(
								"packetLoss_in_percentage" in
								session.rpcs[UUID].stats[stat.trackId]
							)
						) {
							session.rpcs[UUID].stats[
								stat.trackId
							].packetLoss_in_percentage = 0;
						}

						session.rpcs[UUID].stats[stat.trackId].packetLoss_in_percentage =
							session.rpcs[UUID].stats[stat.trackId].packetLoss_in_percentage *
								0.35 +
								(0.65 *
									((stat.packetsLost -
						...
						if (
							session.rpcs[UUID].signalMeter &&
							session.rpcs[UUID].stats[stat.trackId]._type === "video"
						) {
							if (
								session.rpcs[UUID].stats[stat.trackId]
									.packetLoss_in_percentage < 0.01
							) {
								if (
									session.rpcs[UUID].stats[stat.trackId].Bitrate_in_kbps == 0
								) {
					...
					"_codecId" in session.rpcs[UUID].stats &&
					stat.id == session.rpcs[UUID].stats._codecId
				) {
					if ("mimeType" in stat) {
						if (
							...
							session.rpcs[UUID].stats[
								session.rpcs[UUID].stats._codecIdTrackId
							].codec = stat.mimeType;
						} else {
							session.rpcs[UUID].stats[
								session.rpcs[UUID].stats._codecIdTrackId
					...
					if ("frameHeight" in stat) {
						if ("frameWidth" in stat) {
							session.rpcs[UUID].stats.Resolution =
								parseInt(stat.frameWidth) + " x " + parseInt(stat.frameHeight);
						}
					}
				}
			});
			...
			receivers.forEach(function (receiver) {
				try {
					for (var tid in session.rpcs[UUID].stats) {
						if (
							typeof session.rpcs[UUID].stats[tid] == "object" &&
							"_trackID" in session.rpcs[UUID].stats[tid] &&
							session.rpcs[UUID].stats[tid]._trackID === receiver.track.id &&
							"Buffer_Delay_in_ms" in session.rpcs[UUID].stats[tid]
						) {
							var sync_offset = 0.0;
							...
							sync_offset +=
								target_buffer -
								session.rpcs[UUID].stats[tid].Buffer_Delay_in_ms; //  target_Butt
							...
							if (session.rpcs[UUID].stats[tid]._type == "audio") {
								if (receiver.track.id in session.rpcs[UUID].delayNode) {
									...
									var audio_delay =
										video_delay -
										session.rpcs[UUID].stats[tid].Buffer_Delay_in_ms +
										session.sync; // video is typically showing greater delay than video
									if (audio_delay < 0) {
										audio_delay = 0;
									}
									log("audio_delay : " + audio_delay);
									log("audioCtx : " + session.audioCtx.currentTime);
									session.rpcs[UUID].delayNode[
										receiver.track.id
									].delayTime.setValueAtTime(
										parseFloat(audio_delay / 1000.0),
										session.audioCtx.currentTime + 1
									);
									session.rpcs[UUID].stats[tid].Audio_Sync_Delay_ms =
										audio_delay;
								}
		...
		if (typeof obj[key] === "object") {
			...
			if (key.startsWith("_")) {
				...
				try {
					...
					if (key == "useragent") {
						value =
							"<span style='cursor: pointer;' onclick='copyFunction(this.innerText,event);' title='Copy this user-agent to the clipboard' style='cursor:pointer'>" +
							value +
							"</span>";
					}
...
function processMeshcastStats(UUID) {
	try {
		session.rpcs[UUID].mc.getStats().then(function (stats) {
			...
			stats.forEach((stat) => {
				if (stat.type == "candidate-pair" && stat.nominated == true) {
					session.rpcs[UUID].stats["Meshcast Connection"]._local_ice_id =
						stat.localCandidateId;
					session.rpcs[UUID].stats["Meshcast Connection"]._remote_ice_id =
						stat.remoteCandidateId;
					session.rpcs[UUID].stats["Meshcast Connection"].Round_Trip_Time_ms =
					...
					if ("candidateType" in stat) {
						...
						if (stat.candidateType === "relay") {
							...
							try {
								delete session.rpcs[UUID].stats["Meshcast Connection"]
									.local_relayIP;
								delete session.rpcs[UUID].stats["Meshcast Connection"]
									.local_relayProtocol;
							} catch (e) {}
					...
					if ("networkType" in stat) {
						session.rpcs[UUID].stats["Meshcast Connection"].remote_networkType =
							stat.networkType;
					}
				} else if (stat.type == "local-candidate") {
					if (
					...
					if ("candidateType" in stat) {
						...
						if (stat.candidateType === "relay") {
							...
							if ("ip" in stat) {
								session.rpcs[UUID].stats["Meshcast Connection"].local_relayIP =
									stat.ip;
							}
						} else {
							try {
					...
					if ("bytesReceived" in stat) {
						...
						session.rpcs[UUID].stats["Meshcast Connection"]._bytesReceived =
							stat.bytesReceived;
					}
					if ("timestamp" in stat) {
						session.rpcs[UUID].stats["Meshcast Connection"]._timestamp =
							stat.timestamp;
						if (
							!session.rpcs[UUID].stats["Meshcast Connection"]._timestampStart
						) {
							session.rpcs[UUID].stats["Meshcast Connection"]._timestampStart =
								stat.timestamp;
						} else {
	...
	try {
		...
		if (document.querySelector("select#videoSource3")) {
			...
			if (videoSelect.length) {
				...
				if (
					videoSelect[videoSelect.selectedIndex].text.startsWith("OBS-Camera")
				) {
					// OBS Virtualcam
					obscam = true;
				} else if (
					videoSelect[videoSelect.selectedIndex].text.startsWith(
						"OBS Virtual Camera"
					)
				) {
					// OBS Virtualcam
		...
		if (session.streamSrc && session.streamSrc) {
			session.streamSrc.getVideoTracks().forEach(function (track) {
				...
				if (
					obscam &&
					parseInt(session.currentCameraConstraints.frameRate) == 30
				) {
					session.stats.video_settings =
						(session.currentCameraConstraints.width || 0) +
						"x" +
						(session.currentCameraConstraints.height || 0);
				} else {
					...
					if (framerateFPS) {
						session.stats.video_settings =
							...
							(session.currentCameraConstraints.height || 0) +
							" @ " +
							parseInt(framerateFPS * 100) / 100.0 +
							"fps";
					} else {
						session.stats.video_settings =
							(session.currentCameraConstraints.width || 0) +
	...
	function printViewValues(obj) {
		...
		for (var key in obj) {
			if (typeof obj[key] === "object") {
				...
				menu.innerHTML +=
					"<li><span>" + stat + "</span><span>" + value + "</span></li>";
			}
		}
	}
	printViewValues(session.stats);
	menu.innerHTML +=
		"<button onclick='session.forcePLI(null,event);' data-translate='send-keyframe-to-viewer'>Send Keyframe to Viewers</button>";
...
function updateLocalStats() {
					...
					stats.forEach((stat) => {
						if (stat.type == "transport") {
							if ("bytesSent" in stat) {
								if ("_bytesSent" in session.mc.stats) {
									if (session.mc.stats._timestamp) {
										if (stat.timestamp) {
											session.mc.stats.total_sending_bitrate_kbps = parseInt(
												(8 * (stat.bytesSent - session.mc.stats._bytesSent)) /
													(stat.timestamp - session.mc.stats._timestamp)
											);
										}
									}
								}
							...
							if (stat.kind == "video") {
								...
								if ("qualityLimitationReason" in stat) {
									if (session.mc.stats.quality_limitation_reason) {
										if (
											session.mc.stats.quality_limitation_reason !==
								...
								if ("bytesSent" in stat) {
									if ("_bytesSentVideo" in session.mc.stats) {
										if (session.mc.stats._timestamp1) {
											session.mc.stats.video_bitrate_kbps = parseInt(
												(8 *
													(stat.bytesSent - session.mc.stats._bytesSentVideo)) /
													(stat.timestamp - session.mc.stats._timestamp1)
											);
											if (stat.timestamp) {
											}
										}
								...
								if ("nackCount" in stat) {
									if ("_nackCount" in session.mc.stats) {
										if (session.mc.stats._timestamp1) {
											if (stat.timestamp) {
												session.mc.stats.nacks_per_second =
													parseInt(
														(10000 *
															(stat.nackCount - session.mc.stats._nackCount)) /
															(stat.timestamp - session.mc.stats._timestamp1)
													) / 10;
											}
										}
								...
								if ("retransmittedBytesSent" in stat) {
									if ("_retransmittedBytesSent" in session.mc.stats) {
										if (session.mc.stats._timestamp1) {
											if (stat.timestamp) {
												session.mc.stats.retransmitted_kbps = parseInt(
													(8 *
														(stat.retransmittedBytesSent -
															session.mc.stats._retransmittedBytesSent)) /
														(stat.timestamp - session.mc.stats._timestamp1)
												);
											}
										}
								...
								if ("keyFramesEncoded" in stat) {
									session.mc.stats.total_key_frames_encoded =
										stat.keyFramesEncoded;
								}
								...
								if ("bytesSent" in stat) {
									if (session.mc.stats._bytesSentAudio) {
										if (session.mc.stats._timestamp2) {
											if (stat.timestamp) {
												if ("audio_bitrate_kbps" in session.mc.stats) {
													session.mc.stats.audio_bitrate_kbps += parseInt(
														(8 *
															(stat.bytesSent -
																session.mc.stats._bytesSentAudio)) /
															(stat.timestamp - session.mc.stats._timestamp2)
													);
												} else {
													session.mc.stats.audio_bitrate_kbps = 0;
												}
							...
							if ("availableOutgoingBitrate" in stat) {
								session.mc.stats.available_outgoing_bitrate_kbps = parseInt(
									stat.availableOutgoingBitrate / 1024
								);
							}
							if ("totalRoundTripTime" in stat) {
								if ("responsesReceived" in stat) {
									session.mc.stats.average_roundTripTime_ms = parseInt(
										(stat.totalRoundTripTime / stat.responsesReceived) * 1000
									);
								}
							}
						}
				...
				session.pcs[UUID].getStats().then(function (stats) {
					...
					stats.forEach((stat) => {
						if (stat.type == "transport") {
							if ("bytesSent" in stat) {
								if ("_bytesSent" in session.pcs[UUID].stats) {
									if (session.pcs[UUID].stats._timestamp) {
										if (stat.timestamp) {
											session.pcs[UUID].stats.total_sending_bitrate_kbps =
												parseInt(
													(8 *
														(stat.bytesSent -
															session.pcs[UUID].stats._bytesSent)) /
														(stat.timestamp -
															session.pcs[UUID].stats._timestamp)
												);
										}
									}
								}
							...
							if (stat.kind == "video") {
								if ("framesPerSecond" in stat) {
									session.pcs[UUID].stats.resolution =
										stat.frameWidth +
										" x " +
										stat.frameHeight +
										" @ " +
								...
								if ("encoderImplementation" in stat) {
									session.pcs[UUID].stats.video_encoder =
								...
								if ("qualityLimitationReason" in stat) {
									if (session.pcs[UUID].stats.quality_limitation_reason) {
										if (
											session.pcs[UUID].stats.quality_limitation_reason !==
											stat.qualityLimitationReason
										) {
											try {
												var miniInfo = {};
												...
												if ("_hardwareEncoder" in session.pcs[UUID].stats) {
													miniInfo.hw_enc =
														session.pcs[UUID].stats._hardwareEncoder;
												} else {
													miniInfo.hw_enc = null;
												}
								...
								if ("bytesSent" in stat) {
									if ("_bytesSentVideo" in session.pcs[UUID].stats) {
										if (session.pcs[UUID].stats._timestamp1) {
											session.pcs[UUID].stats.video_bitrate_kbps = parseInt(
												(8 *
													(stat.bytesSent -
														session.pcs[UUID].stats._bytesSentVideo)) /
													(stat.timestamp - session.pcs[UUID].stats._timestamp1)
											);
											if (stat.timestamp) {
											}
										}
								...
								if ("nackCount" in stat) {
									if ("_nackCount" in session.pcs[UUID].stats) {
										if (session.pcs[UUID].stats._timestamp1) {
											if (stat.timestamp) {
												session.pcs[UUID].stats.nacks_per_second =
													parseInt(
														(10000 *
															(stat.nackCount -
																session.pcs[UUID].stats._nackCount)) /
															(stat.timestamp -
																session.pcs[UUID].stats._timestamp1)
													) / 10;
											}
										}
									}
								...
								if ("retransmittedBytesSent" in stat) {
									if ("_retransmittedBytesSent" in session.pcs[UUID].stats) {
										if (session.pcs[UUID].stats._timestamp1) {
											if (stat.timestamp) {
												session.pcs[UUID].stats.retransmitted_kbps = parseInt(
													(8 *
														(stat.retransmittedBytesSent -
															session.pcs[UUID].stats
																._retransmittedBytesSent)) /
														(stat.timestamp -
															session.pcs[UUID].stats._timestamp1)
												);
											}
								...
								if ("retransmittedBytesSent" in stat) {
									session.pcs[UUID].stats._retransmittedBytesSent =
										stat.retransmittedBytesSent;
								}
								...
								if ("keyFramesEncoded" in stat) {
									session.pcs[UUID].stats.total_key_frames_encoded =
										stat.keyFramesEncoded;
								}
							} else if (stat.kind == "audio") {
								if ("bytesSent" in stat) {
									if (session.pcs[UUID].stats._bytesSentAudio) {
										if (session.pcs[UUID].stats._timestamp2) {
											if (stat.timestamp) {
												if ("audio_bitrate_kbps" in session.pcs[UUID].stats) {
													session.pcs[UUID].stats.audio_bitrate_kbps +=
														parseInt(
															(8 *
																(stat.bytesSent -
																	session.pcs[UUID].stats._bytesSentAudio)) /
																(stat.timestamp -
																	session.pcs[UUID].stats._timestamp2)
														);
												} else {
													session.pcs[UUID].stats.audio_bitrate_kbps = 0;
												}
							...
							if ("candidateType" in stat) {
								session.pcs[UUID].stats.remote_candidateType =
									stat.candidateType;
								if (stat.candidateType === "relay") {
									if ("ip" in stat) {
									...
									if ("relayProtocol" in stat) {
										session.pcs[UUID].stats.remote_relayProtocol =
											stat.relayProtocol;
									}
								} else {
									try {
							...
							if ("candidateType" in stat) {
								session.pcs[UUID].stats.local_candidateType =
									stat.candidateType;
								...
								if (stat.candidateType === "relay") {
									...
									if ("relayProtocol" in stat) {
										session.pcs[UUID].stats.local_relayProtocol =
											stat.relayProtocol;
									}
								} else {
									try {
							...
							if ("availableOutgoingBitrate" in stat) {
								session.pcs[UUID].stats.available_outgoing_bitrate_kbps =
									parseInt(stat.availableOutgoingBitrate / 1024);
							}
							if ("totalRoundTripTime" in stat) {
								if ("responsesReceived" in stat) {
									session.pcs[UUID].stats.average_roundTripTime_ms = parseInt(
										(stat.totalRoundTripTime / stat.responsesReceived) * 1000
									);
								}
							}
	...
	try {
		...
		/* if (meshcastActive) {
			if (totalAudio) {
				headerStats +=
					"</span>, <span title='Number of outbound audio streams'>👂 " +
					totalAudio;
			}
			if (totalVideo) {
				headerStats +=
					"</span>, <span title='Number of outbound video streams'>👀 " +
					totalVideo;
			}
			headerStats +=
				"</span>, <span title='Publishing to a Meshcast broadcasting server''>📡Broadcast";
		} else {
			headerStats +=
				"</span>, <span title='Number of outbound audio streams'>👂 " +
				totalAudio;
		...
		/* if (session.roomid) {
			headerStats +=
				"</span>, <span title='Number of scenes.'>🎬 " +
				totalScenes +
				"</span>";
		} */
	} catch (e) {}
	...
	if (Firefox) {
		...
		headerStats +=
			"<span title='Total upload bitrate'>🔼 " +
			Math.round(totalBitrate2 / 10.24) / 100 +
			"<small>-mbps</small></span>";
	} else {
		headerStats +=
			"<span title='Total upload bitrate'>🔼 " +
			totalBitrate2 +
			"<small>-kbps</small></span>";
	}
	if (cpuLimited) {
		headerStats +=
			"<span style='color: #e69a0f;' title='Your CPU is maxed out; this can cause audio, sync, and quality issues.'>🔥 CPU Overloaded</span>";
	}
...
function toggleControlBar() {
	if (getById("controlButtons").style.display != "none") {
		...
		getById("controlButtons").dataset.style =
			getById("controlButtons").style.display;
...
function toggleMute(apply = false) {
	...
	if (session.muted == false) {
		session.muted = true;
		getById("mutetoggle").className =
			"las la-microphone-slash my-float toggleSize";
		if (!session.cleanOutput) {
	...
	postMessageIframe(document.getElementById("screensharesource"), {
		mic: !session.muted,
	});
...
function toggleSpeakerMute(apply = false) {
	...
	if (session.speakerMuted == false) {
		...
		getById("mutespeakertoggle").className =
			"las la-volume-mute my-float toggleSize";
		if (!session.cleanOutput) {
		...
		getById("mutespeakertoggle").className =
			"las la-volume-up my-float toggleSize";
		if (!session.cleanOutput) {
	...
	for (var UUID in session.rpcs) {
		applyMuteState(UUID);
		postMessageIframe(session.rpcs[UUID].iframeEle, {
			mute: session.speakerMuted,
		});
	}
...
function toggleFileshare(UUID = false, event = null) {
	if (UUID === false) {
		var string =
			'Share a file with the group<br /><input id="fileselector3" onchange="session.shareFile(this, false, event);" type="file" title="Transfer any file to the group"/><div id="activeShares"></div>';
	} else if (session.directorList.indexOf(UUID) >= 0) {
		var string =
			'The director requested you share a file with them.<br /><input id="fileselector3" onchange="session.shareFile(this, `' +
			UUID +
			'`, event);" type="file" title="Transfer a file to the director"/><div id="activeShares"></div>';
	} else {
		var string =
			'Someone has requested you share a file with them.<br /><input id="fileselector3" onchange="session.shareFile(this, `' +
			UUID +
			'`, event);" type="file" title="Transfer a file to person"/><div id="activeShares"></div>';
	}
	warnUser(string);
	if (session.hostedFiles) {
		if (session.hostedFiles.length) {
			getById("activeShares").innerHTML +=
				"<div><u>Files being shared:</u></div>";
		}
		for (var i = 0; i < session.hostedFiles.length; i++) {
			...
			getById("activeShares").innerHTML +=
				"<div><b>" +
				session.hostedFiles[i].name +
				"</b> (" +
				Math.ceil(session.hostedFiles[i].size / ((1024 * 1024) / 10)) / 10 +
				"-MB)</div>";
		}
	}
	if (session.hostedTransfers) {
		getById("activeShares").innerHTML +=
			"<div><i>" +
			session.hostedTransfers.length +
			" file transfers in progress.</i></div>";
	}
}
...
function directorAdvanced(ele) {
	...
	target.style =
		"position:absolute;float:left;width:270px;height:222px;background-color:#7E7E7E;";
...
function directorSendMessage(ele) {
	...
	sendButton.innerHTML =
		"<i class='las la-reply'></i> <span data-translate='send-message'>send message<s/pan> ";
	...
	closeButton.innerHTML =
		"<i class='las la-times'></i> <span data-translate='close'>close</span>";
	...
	overlayMsg.onclick = function (e) {
		...
		if (e.target.parentNode.parentNode.overlay === true) {
			...
			e.target.parentNode.innerHTML =
				"<i class='las la-bell-slash' style='font-size:170%; color:#DDD; cursor:pointer;'></i>";
		} else {
...
function toggleVideoMute(apply = false) {
	...
	if (session.videoMuted == false) {
		...
		getById("mutevideotoggle").className =
			"las la-video-slash my-float toggleSize";
		if (!session.cleanOutput) {
...
function toggleSettings(forceShow = false) {
	// TODO: I need to have this be MUTE, toggle, with volume not touched.
		...
		confirmAlt(
			...
			if (res) {
				getById("main").innerHTML =
					"<font style='font-size:500%;text-align:center;margin:auto;'>👋</font>";
				setTimeout(function () {
					session.hangup();
				}, 0);
			}
...
function openSettingsWaterMark() {
	document.getElementById("popupWaterMarker").classList.toggle("show");
}
...
function hangup2() {
	...
	if (session.showDirector == false) {
		getById("miniPerformer").innerHTML =
			'<button id="press2talk" onmousedown="event.preventDefault(); event.stopPropagation();" style="width:auto;margin-left:5px;height:45px;border-radius: 38px;" class="float" onclick="press2talk(true);" title="You can also enable the director`s Video Output afterwards by clicking the Setting`s button"><i class="las la-headset"></i><span data-translate="push-to-talk-enable"> enable director`s microphone or video<br />(only guests can see this feed)</span></button>';
...
function hangupComplete() {
	getById("main").innerHTML =
		"<font style='font-size:200%;text-align:center;margin:auto;color:#fff'>¡Te has desconectado!</font> <br />";
...
async function directMigrate(ele, event, room = false) {
	...
	if (room) {
		...
		if (previousRoom === null) {
			...
			ele.innerHTML =
				'<i class="las la-paper-plane"></i> <span data-translate="forward-to-room">transfer</span>';
			miniTranslate(ele);
		...
		ele.innerHTML =
			'<i class="las la-check"></i> <span data-translate="forward-to-room">armed</span>';
		miniTranslate(ele);
		...
		if (armedTransfer !== false && previousRoom !== "") {
			...
			if ("broadcast" in transferSettings) {
				...
				session.rpcs[ele.dataset.UUID] &&
				session.rpcs[ele.dataset.UUID].stats.info &&
				"broadcast_mode" in session.rpcs[ele.dataset.UUID].stats.info
			) {
				broadcastMode =
					session.rpcs[ele.dataset.UUID].stats.info.broadcast_mode;
			}
			...
			var response = await promptTransfer(
				previousRoom,
				broadcastMode,
				updateurl
			);
...
function directHangup(ele, event) {
	...
	if (confirmHangup) {
		...
		ele.innerHTML =
			'<i class="las la-sign-out-alt"></i><span data-translate="disconnect-guest"> Hangup</span>';
		miniTranslate(ele);
		ele.style.backgroundColor = null;
	}
}
...
function directEnable(ele, event, director = false) {
	...
	if (!(event.ctrlKey || event.metaKey)) {
		if (ele.dataset.value == 1) {
			...
			if (director) {
				if (
					getById("container_director").querySelectorAll(
						'[data-action-type="addToScene"][data-value="1"]'
					).length == 0
				) {
				...
				if (
					getById("container_" + ele.dataset.UUID).querySelectorAll(
						'[data-action-type="addToScene"][data-value="1"]'
					).length == 0
				) {
			...
			if (director) {
				...
				getById("container_" + ele.dataset.UUID).style.backgroundColor =
					"#649166";
			}
		}
	}
	...
	for (var uuid in session.pcs) {
		if (
			session.pcs[uuid].stats.info &&
			"version" in session.pcs[uuid].stats.info &&
			session.pcs[uuid].stats.info.version < 17.2
		) {
...
function issueLayout(layout = false, scene = false) {
	// A directing room only is controlled by the Director, with the exception of MUTE.
	log("issueLayout()");
	var msg = {};
	msg.action = "layout";
...
async function directPageReload(ele, event) {
	...
	if (event === false) {
		if (previousURL === null) {
			// user cancelled in previous callback
			ele.innerHTML =
				'<i class="las la-sync"></i> <span data-translate="change-url">change URL</span>';
			miniTranslate(ele);
			ele.style.backgroundColor = null;
			return;
		}
		...
		ele.innerHTML =
			'<i class="las la-check"></i> <span data-translate="button-armed">armed</span>';
		miniTranslate(ele);
		ele.style.backgroundColor = "#BF3F3F";
		...
		window.focus();
		var reloadURL = await promptAlt(
			miscTranslations["transfer-guest-to-url"],
			false,
			false,
			previousURL
		);
...
async function directTimer(ele, event = false) {
	// A directing room only is controlled by the Director, with the exception of MUTE.
	log("directTimer");
	if (!ele.dataset.UUID) {
		return;
	...
	if (!event || !(event.ctrlKey || event.metaKey)) {
		if (ele.dataset.value == 0 || ele.dataset.value == 2) {
			var getTime = await promptAlt(
				"Time in seconds to count down",
				false,
				false,
				parseInt(getById("overlayClockContainer").dataset.initial)
			);
			if (!getTime) {
				return;
			}
			getById("overlayClockContainer").dataset.initial =
				parseInt(getTime) || 600;
			ele.dataset.value = 1;
			ele.classList.add("pressed");
			ele.classList.remove("red2");
			...
			msg.startClock = true;
			ele.innerHTML =
				'<i class="las la-clock"></i><span data-translate="create-timer"> Remove Timer</span>';
			...
			msg.hideClock = true;
			ele.innerHTML =
				'<i class="las la-clock"></i><span data-translate="create-timer"> Create Timer</span>';
		}
		//miniTranslate(ele);
	} else if (event.ctrlKey || event.metaKey) {
		if (ele.dataset.value == 1) {
...
function updateRemoteTimerButton(UUID, currentTime) {
	var elements = document.querySelectorAll(
		'[data-action-type="create-timer"][data--u-u-i-d="' + UUID + '"]'
	);
	if (elements[0]) {
...
function directMute(ele, event = false) {
	...
	if (!event || !(event.ctrlKey || event.metaKey)) {
		if (ele.dataset.value == 0) {
			...
			ele.classList.remove("pressed");
			ele.innerHTML =
				'<i class="las la-microphone-slash"></i> <span data-translate="mute-scene" >mute in scene</span>';
		} else {
			ele.dataset.value = 0;
			ele.classList.add("pressed");
			ele.innerHTML =
				'<i class="las la-microphone-slash"></i> <span data-translate="unmute" >un-mute</span>';
		}
		miniTranslate(ele);
	}
	var msg = {};
...
function requestFileUpload(ele) {
	...
	ele.disabled = true;
	ele.innerHTML =
		'<i class="las la-file-upload"></i><span data-translate="request-upload"> Requesting..</span>';
	setTimeout(
		function (ele) {
			try {
				ele.innerHTML =
					'<i class="las la-file-upload"></i><span data-translate="request-upload"> Request File</span>';
				ele.classList.remove("pressed");
				ele.disabled = false;
			} catch (e) {}
		},
...
function remoteSpeakerMute(ele, event = false) {
	...
	if (!event || !(event.ctrlKey || event.metaKey)) {
		if (ele.dataset.value == 1) {
			...
			ele.classList.add("pressed");
			ele.innerHTML =
				'<i class="las la-volume-off"></i> <span data-translate="undeafen">un-deafen</span>';
		}
		miniTranslate(ele);
	}
...
function updateRemoteSpeakerMute(UUID) {
	var ele = document.querySelectorAll(
		'[data-action-type="toggle-remote-speaker"][data--u-u-i-d="' + UUID + '"]'
	);
	if (ele[0]) {
...
function updateRemoteDisplayMute(UUID, blind = true) {
	var ele = document.querySelectorAll(
		'[data-action-type="toggle-remote-display"][data--u-u-i-d="' + UUID + '"]'
	);
	if (ele[0]) {
		if (blind) {
			ele[0].classList.add("pressed");
			ele[0].dataset.value = 1;
			ele[0].innerHTML =
				'<i class="las la-eye-slash"></i> <span data-translate="unblind">un-blind</span>';
			miniTranslate(ele[0]);
		} else {
			ele[0].classList.remove("pressed");
			ele[0].dataset.value = 0;
			ele[0].innerHTML =
				'<i class="las la-eye"></i> <span data-translate="blind">blind</span>';
			miniTranslate(ele[0]);
		}
	}
}
...
function remoteMuteVideo(ele, event = false) {
	...
	if (!event || event.ctrlKey || event.metaKey) {
		...
		if (ele.dataset.value == 1) {
			...
			ele.classList.remove("pressed");
			ele.innerHTML =
				'<i class="las la-video-slash"></i> <span data-translate="hide-guest" >hide guest</span>';
		} else {
			ele.dataset.value = 1;
			ele.classList.add("pressed");
			ele.innerHTML =
				'<i class="las la-video-slash"></i> <span data-translate="unhide-guest" >un-hide</span>';
		}
		miniTranslate(ele);
		ele.style.backgroundColor = null;
	}
...
function updateDirectorVideoMute(UUID) {
	var ele = document.querySelectorAll(
		'[data-action-type="hide-guest"][data--u-u-i-d="' + UUID + '"]'
	);
	if (ele[0]) {
		ele[0].dataset.value = 1;
		ele[0].classList.add("pressed");
...
function applyMuteState(UUID) {
	...
	var muteOutcome =
		session.rpcs[UUID].mutedState ||
		session.rpcs[UUID].mutedStateMixer ||
		session.rpcs[UUID].mutedStateScene ||
		session.speakerMuted ||
		session.rpcs[UUID].bandwidthMuted;
	if (session.rpcs[UUID].videoElement) {
		if (
			session.rpcs[UUID].videoElement &&
			session.rpcs[UUID].videoElement.usermuted === true
		) {
			return "usermuted true";
		}
...
function saveDirectorSettings() {
	...
	for (var i = 0; i < customizeLinks1.length; i++) {
		settings.customizeLinks1[customizeLinks1[i].dataset.param] =
			customizeLinks1[i].checked;
	}
	...
	for (var i = 0; i < customizeLinks3.length; i++) {
		settings.customizeLinks3[customizeLinks3[i].dataset.param] =
			customizeLinks3[i].checked;
	}
	...
	for (var i = 0; i < directorLinks1.length; i++) {
		settings.directorLinks1[directorLinks1[i].dataset.param] =
			directorLinks1[i].checked;
	}
	...
	for (var i = 0; i < directorLinks2.length; i++) {
		settings.directorLinks2[directorLinks2[i].dataset.param] =
			directorLinks2[i].checked;
	}
...
function loadDirectorSettings() {
	...
	if (settings.customizeLinks1) {
		...
		Object.keys(settings.customizeLinks1).forEach((key, index) => {
			try {
				if (
					customizeLinks1.querySelector('[data-param="' + key + '"]').checked !=
					settings.customizeLinks1[key]
				) {
					customizeLinks1.querySelector('[data-param="' + key + '"]').checked =
						settings.customizeLinks1[key];
					customizeLinks1
						.querySelector('[data-param="' + key + '"]')
						.onchange();
				}
			} catch (e) {
				errorlog(e);
	...
	if (settings.customizeLinks3) {
		...
		Object.keys(settings.customizeLinks3).forEach((key, index) => {
			try {
				if (
					customizeLinks3.querySelector('[data-param="' + key + '"]').checked ==
					settings.customizeLinks3[key]
				) {
					customizeLinks3.querySelector('[data-param="' + key + '"]').checked =
						settings.customizeLinks3[key];
					customizeLinks3
	...
	if (settings.directorLinks1) {
		...
		Object.keys(settings.directorLinks1).forEach((key, index) => {
			try {
				if (
					directorLinks1.querySelector('[data-param="' + key + '"]').checked ==
					settings.directorLinks1[key]
				) {
					directorLinks1.querySelector('[data-param="' + key + '"]').checked =
						settings.directorLinks1[key];
	...
	if (settings.directorLinks2) {
		...
		Object.keys(settings.directorLinks2).forEach((key, index) => {
			try {
				if (
					directorLinks2.querySelector('[data-param="' + key + '"]').checked ==
					settings.directorLinks2[key]
				) {
					directorLinks2.querySelector('[data-param="' + key + '"]').checked =
						settings.directorLinks2[key];
...
function sendChat(chatmessage = "hi", UUID = false) {
	// A directing room only is controlled by the Director, with the exception of MUTE.
	log("Chat message");
	var msg = {};
...
function publishScreen() {
	...
	var quality = parseInt(
		getById("webcamquality2").elements.namedItem("resolution2").value
	);
	...
	publishScreen2(constraints, audioSelect)
		.then((res) => {
			if (res == false) {
				return;
			} // no screen selected
	...
	if (iframeURL.startsWith("https://") || iframeURL.startsWith("http://")) {
		...
		if (
			...
			if (vidid) {
				...
				var regExp =
					/^.*((youtu.be\/)|(v\/)|(\/u\/\w\/)|(embed\/)|(videoseries\?))\??list?=?([^#&?]*).*/;
				var match = iframeURL.match(regExp);
				...
				if (plid) {
					iframeURL =
						...
						plid +
						"&autoplay=1&modestbranding=1&playsinline=1&enablejsapi=1";
				}
			}
			...
			if (iframeURL.includes("twitch.tv/popout/")) {
				...
				if (iframeURL.includes("darkpopout=")) {
					iframeURL = iframeURL.replace(
						"?darkpopout=",
						"?darkpopout=&parent=" + location.hostname
					);
				} else {
					iframeURL = iframeURL.replace(
						"?darkpopout",
...
function YoutubeListen(iframe_id) {
	...
	try {
		iframe.contentWindow.postMessage(
			JSON.stringify({ event: "listening", id: iframe_id }),
			"*"
		); //
	} catch (e) {}
	setTimeout(
		...
		try {
			...
			if ("playerState" in data.info) {
				...
				if ("videoData" in data.info) {
					if (
						session.iframeEle.sendOnNewConnect.ifs.v !=
						...
						if (vidSrc !== session.iframeSrc) {
							...
							if (parseInt(msg.ifs.t) > 1) {
								data.iframeSrc +=
									"&start=" + parseInt(Math.ceil(msg.ifs.t)) + "";
							}
...
function processIframeSyncUpdates(ifs, UUID) {
	...
	if (ifs.r) {
		/// setPlaybackRate
		YoutubeAPI(session.rpcs[UUID].iframeEle, "setPlaybackRate", [
			parseFloat(ifs.r),
		]);
	}
	if ("s" in ifs) {
		...
		if (tracks.length) {
			...
			if (tracks.length > 1) {
				...
				try {
					...
					webAudio.mediaStreamSource = audioContext.createMediaStreamSource(
						session.streamSrc
					);
					webAudio.gainNode = audioGainNode(
						webAudio.mediaStreamSource,
						audioContext
					);
				}
			} else {
				...
				session.videoElement.srcObject
					...
					.forEach(function (track) {
						if (webAudio.id != track.id) {
							webAudio.destination.stream.addTrack(
								track,
								session.videoElement.srcObject
							);
						}
...
function changeLowCut(freq, deviceid = null) {
	...
	for (var webAudio in session.webAudios) {
		...
		session.webAudios[webAudio].lowcut1.frequency.setValueAtTime(
			freq,
			session.webAudios[webAudio].audioContext.currentTime
		);
		session.webAudios[webAudio].lowcut2.frequency.setValueAtTime(
			freq,
			session.webAudios[webAudio].audioContext.currentTime
		);
		session.webAudios[webAudio].lowcut3.frequency.setValueAtTime(
			freq,
			session.webAudios[webAudio].audioContext.currentTime
		);
	}
...
function changeLowEQ(lowEQ, deviceid = null) {
	...
	for (var webAudio in session.webAudios) {
		...
		session.webAudios[webAudio].lowEQ.gain.setValueAtTime(
			lowEQ,
			session.webAudios[webAudio].audioContext.currentTime
		);
	}
...
function changeMidEQ(midEQ, deviceid = null) {
	for (var webAudio in session.webAudios) {
		...
		session.webAudios[webAudio].midEQ.gain.setValueAtTime(
			midEQ,
			session.webAudios[webAudio].audioContext.currentTime
		);
	}
...
function changeHighEQ(highEQ, deviceid = null) {
	for (var webAudio in session.webAudios) {
		...
		session.webAudios[webAudio].highEQ.gain.setValueAtTime(
			highEQ,
			session.webAudios[webAudio].audioContext.currentTime
		);
	}
	...
	for (var webAudio in session.webAudios) {
		try {
			...
			if (deviceid in session.webAudios[webAudio].subGainNodes) {
				session.webAudios[webAudio].subGainNodes[deviceid].gain.setValueAtTime(
					gain,
					session.webAudios[webAudio].audioContext.currentTime
				);
			} else {
...
function changeMainGain(gain, deviceid = null) {
	for (var webAudio in session.webAudios) {
		...
		session.webAudios[webAudio].gainNode.gain.setValueAtTime(
			gain,
			session.webAudios[webAudio].audioContext.currentTime
		);
	}
...
function audioMeter(mediaStreamSource, audioContext) {
	...
	function draw() {
		...
		if (document.getElementById("meter1")) {
			...
			document.getElementById("mutetoggle").style.color =
				"rgb(" + (255 - total) + ",255," + (255 - total) + ")";
		} else {
...
function activeSpeaker(border = false) {
	...
	for (var UUID in session.rpcs) {
		if (session.rpcs[UUID].stats._Audio_Loudness_average) {
			...
			if (
				session.rpcs[UUID].stats.Audio_Loudness &&
				session.rpcs[UUID].stats.Audio_Loudness > 10
			) {
				session.rpcs[UUID].stats._Audio_Loudness_average = parseFloat(
					session.rpcs[UUID].stats.Audio_Loudness * 0.07 +
						session.rpcs[UUID].stats._Audio_Loudness_average * 0.93
				);
			} else {
				session.rpcs[UUID].stats._Audio_Loudness_average = parseFloat(
					session.rpcs[UUID].stats._Audio_Loudness_average * 0.975
				);
			}
		} else {
		...
		if (
			session.rpcs[UUID].stats.Audio_Loudness > 13 ||
			(session.rpcs[UUID].stats.Audio_Loudness > 5 &&
				session.rpcs[UUID].stats._Audio_Loudness_average > 3) ||
			session.rpcs[UUID].stats._Audio_Loudness_average > 6
		) {
	...
	if (session.activeSpeaker === 1) {
		if (!anyoneIsSpeaking) {
			if (defaultSpeaker) {
				...
				session.scene === false ||
				(session.nopreview === false && session.minipreview !== 1)
			) {
				...
				for (var UUID in session.rpcs) {
					if (
						session.rpcs[UUID].videoElement &&
						session.rpcs[UUID].videoElement.srcObject &&
						session.rpcs[UUID].videoElement.srcObject.getVideoTracks().length
					) {
			...
			for (var UUID in session.rpcs) {
				...
				if (!loudest) {
					...
					session.rpcs[UUID].stats._Audio_Loudness_average >
					session.rpcs[loudest].stats._Audio_Loudness_average
				) {
					loudest = UUID;
				}
				...
				if (session.rpcs[UUID].activelySpeaking) {
					if (!loudestActive) {
						...
						session.rpcs[UUID].stats._Audio_Loudness_average >
						session.rpcs[loudestActive].stats._Audio_Loudness_average
					) {
		...
		if (!anyoneIsSpeaking) {
			...
			for (var UUID in session.rpcs) {
				if (
					session.rpcs[UUID].activelySpeaking &&
					!session.rpcs[UUID].defaultSpeaker
				) {
					session.rpcs[UUID].defaultSpeaker = true;
					changed = true;
				} else if (
						...
						if (session.defaultPassword === false) {
							if (session.password === false) {
								var invite =
									...
									"&password=false";
								warnUser(
									"You can invite others with:\n\n<a target='_blank' title='Copy this link to the clipboard' style='cursor:pointer' onclick='copyFunction(this.innerText,event);' href='" +
										invite +
										"'>" +
										invite +
										"</a>"
								...
								generateHash(session.password + session.salt, 4).then(function (
									hash
								) {
									var invite =
										"https://" +
								...
								if (response[i].UUID in session.rpcs) {
									...
									if (session.queue) {
										if (session.directorList.indexOf(response[i].UUID) >= 0) {
											...
											session.view_set &&
											session.view_set.includes(streamID)
										) {
											play(streamID, response[i].UUID);
										} else if (session.queueList.length < 5000) {
											if (
												!(streamID in session.watchTimeoutList) &&
												!session.queueList.includes(streamID)
											) {
...
function hideDirectorinvites(ele, skip = true) {
	if (getById("directorLinks2").style.display == "none") {
		...
		ele.innerHTML =
			'<i class="las la-caret-right"></i><span data-translate="hide-the-links"> LINKS (GUEST INVITES & SCENES)</span>';
...
async function toggleCoDirector(ele) {
	...
	if (!session.directorPassword) {
		session.directorPassword = await promptAlt(
			miscTranslations["enter-new-codirector-password"],
			false
		);
		if (!session.directorPassword) {
	...
	getById("codirectorSettings_invite").value =
		"https://" +
		location.host +
		location.pathname +
		"?dir=" +
...
function createRoomCallback(passAdd, passAdd2) {
	...
	if (session.customWSS) {
		if (session.customWSS !== true) {
			// to be deprecated, as piesocket is no longer free.
	...
	if (session.cleanDirector == false && session.cleanOutput == false) {
		...
		getById("director_block_1").dataset.raw =
			"https://" +
			location.host +
			location.pathname +
			"?room=" +
		...
		getById("director_block_1").href =
			...
			pie +
			queue;
		getById("director_block_1").innerText =
			"https://" +
			location.host +
			location.pathname +
			"?room=" +
		...
		getById("director_block_3").dataset.raw =
			"https://" +
			location.host +
			location.pathname +
			"?scene&room=" +
		...
		getById("director_block_3").href =
			...
			passAdd2 +
			pie;
		getById("director_block_3").innerText =
			"https://" +
			location.host +
			location.pathname +
			"?scene&room=" +
	...
	if (!session.cleanOutput) {
		...
		if (session.showDirector == false) {
			getById("miniPerformer").innerHTML =
				'<button id="press2talk" onmousedown="event.preventDefault(); event.stopPropagation();" style="width:auto;margin-left:5px;height:45px;border-radius: 38px;" class="float" onclick="press2talk(true);" title="You can also enable the director`s Video Output afterwards by clicking the Setting`s button"><i class="las la-headset"></i><span data-translate="push-to-talk-enable"> enable director`s microphone or video<br />(only guests can see this feed)</span></button>';
			miniTranslate(getById("miniPerformer"));
			getById("grabDirectorSoloLink").dataset.raw =
				"https://" +
				location.host +
				location.pathname +
				"?scn&sd&r=" +
				...
				passAdd2 +
				pie;
			getById("grabDirectorSoloLink").href =
				"https://" +
				location.host +
				location.pathname +
				"?scn&sd&r=" +
				...
				passAdd2 +
				pie;
			getById("grabDirectorSoloLink").innerText =
				"https://" +
				location.host +
				location.pathname +
				"?scn&sd&r=" +
		...
		if (session.rooms && session.rooms.length > 0) {
			...
			session.rooms.forEach(function (r) {
				...
				container.innerHTML +=
					'<button id="roomselect_' +
					r +
					'" onmousedown="event.preventDefault(); event.stopPropagation();" class="float btnArmTransferRoom" onclick="handleRoomSelect(\'' +
					r +
					'\');" title="Arm/disarm transfer to this room" tabindex="' +
					tabindex +
	...
	if (
		typeof directorWebsiteShare === "object" &&
		directorWebsiteShare !== null &&
		"website" in directorWebsiteShare
	) {
		if (directorWebsiteShare.website == false) {
			clearDirectorSettings();
		} else if (
			directorWebsiteShare.roomid &&
			directorWebsiteShare.roomid == session.roomid
		) {
			session.iframeSrc = directorWebsiteShare.website;
			session.defaultIframeSrc = directorWebsiteShare.website;

			getById("websitesharebutton").classList.add("advanced");
			getById("websitesharebutton2").classList.remove("advanced");
		}
	}
...
function getDirectorSettings(scene) {
	...
	if (scene) {
		var eles = document.querySelectorAll(
			'[data-action-type="addToScene"][data-scene="' + scene + '"'
		);
		settings.scene = {};
		for (var i = 0; i < eles.length; i++) {
			if (parseInt(eles[i].dataset.value) == 1) {
	...
	for (var i = 0; i < eles.length; i++) {
		if (parseInt(eles[i].dataset.value) == 0) {
			// if muted
			if (eles[i].dataset.sid) {
				var msg = {};
				msg.action = "mute";
...
async function createDirectorOnlyBox() {
	...
	var buttons =
		"<div title='Does not impact scene order.' class='shift'><i class='las la-angle-left' onclick='shiftPC(this,-1, true);'></i><i class='las la-angle-right' onclick='shiftPC(this,1, true)';></i></div><div class='streamID' style='user-select: none;'>ID: <span style='user-select: text;'>" +
		session.streamID +
		"</span>\
	...
	<span id='label_director' title='Click here to edit the label for this stream. Changes will propagate to all viewers of this stream' data-translate='add-a-label'>" +
		miscTranslations["add-a-label"] +
		"</span>\
		...
		if (session.showDirector) {
			if (document.getElementById("container_director")) {
				if (
					...
					newScene.innerHTML =
						'<button style="margin: 0 5px 10px 5px;" data-sid="' +
						session.streamID +
						'" data-action-type="addToScene" data-scene="' +
						scene +
						'"   title="Add to Scene ' +
						scene +
						'" onclick="directEnable(this, event);"><span ><i class="las la-plus-square" style="color:#060"></i> Scene: ' +
						scene +
						"</span></button>";
	...
	labelID.onclick = async function (ee) {
		...
		window.focus();
		var newlabel = await promptAlt(
			miscTranslations["enter-new-display-name"],
			false,
			false,
			oldlabel
		);
			...
			if (getById("guestFeeds")) {
				var currentPosition =
					Array.prototype.indexOf.call(
						getById("guestFeeds").children,
						document.getElementById("container_" + UUID)
					) + 1;
				...
				while (
					currentPosition < parseInt(ele.dataset.locked) &&
					getById("guestFeeds").children.length > currentPosition
				) {
					var node = document.getElementById("container_" + UUID);
					(parent = node.parentNode),
...
function createControlBox(UUID, soloLink, streamID) {
	...
	if (session.orderby) {
		try {
			...
			for (var i = 0; i < getById("guestFeeds").children.length; i++) {
				if (
					getById("guestFeeds").children[i].UUID &&
					!getById("guestFeeds").children[i].shifted
				) {
					if (getById("guestFeeds").children[i].UUID in session.rpcs) {
						if (
							session.rpcs[
								getById("guestFeeds").children[i].UUID
							].streamID.toLowerCase() > streamID.toLowerCase()
						) {
							getById("guestFeeds").insertBefore(
								container,
								getById("guestFeeds").children[i]
							);
							added = true;
							break;
						}
	...
	<span id='label_" +
		UUID +
		"' title='Click here to edit the label for this stream. Changes will propagate to all viewers of this stream'></span>\
	</div>";

	if (!session.rpcs[UUID].voiceMeter) {
		if (session.meterStyle == 1) {
			// director specific style
			session.rpcs[UUID].voiceMeter = getById("voiceMeterTemplate2").cloneNode(
				true
			);
		} else {
			session.rpcs[UUID].voiceMeter =
				getById("voiceMeterTemplate").cloneNode(true);
	...
	session.rpcs[UUID].remoteMuteElement =
		getById("muteStateTemplate").cloneNode(true);
	...
	session.rpcs[UUID].remoteVideoMuteElement = getById(
		"videoMuteStateTemplate"
	).cloneNode(true);
	...
	session.rpcs[UUID].remoteRaisedHandElement =
		getById("raisedHandTemplate").cloneNode(true);
	...
	controls.innerHTML +=
		"<div style='margin:10px;' id='advanced_audio_director_" +
		UUID +
		"' class='advanced'></div>";
	controls.innerHTML +=
		"<div style='margin:10px;' id='advanced_video_director_" +
		UUID +
		"' class='advanced'></div>";
	...
	controls.innerHTML +=
		...
		handsID +
		"' style='margin: auto;margin-bottom:10px;display:none;background-color:yellow;' data-value='0' title=\"This guest raised their hand. Click this to clear notification.\" onclick=\"remoteLowerhands('" +
		UUID +
		'\');">\
	...
	if (session.signalMeter) {
		if (!session.rpcs[UUID].signalMeter) {
			session.rpcs[UUID].signalMeter = getById("signalMeterTemplate").cloneNode(
				true
			);
			...
			if (
				session.rpcs[UUID].stats.info &&
				session.rpcs[UUID].stats.info.cpuLimited
			) {
		...
		if (!session.rpcs[UUID].batteryMeter) {
			session.rpcs[UUID].batteryMeter = getById(
				"batteryMeterTemplate"
			).cloneNode(true);
			...
			if (
				...
				var level =
					session.rpcs[UUID].batteryMeter.querySelector(".battery-level");
				if (level) {
			...
			if (
				session.rpcs[UUID].stats.info &&
				"plugged_in" in session.rpcs[UUID].stats.info &&
				session.rpcs[UUID].stats.info.plugged_in === false
			) {
...
function cycleCameras() {
	...
	var matched = false;
	var maxIndex =
		parseInt(getById("flipcamerabutton").dataset.maxIndex) ||
		parseInt(videoSelect.length);
...
function press2talk(clean = false) {
	...
	if (
		session.videoDevice ||
		(session.audioDevice && session.audioDevice !== 1)
	) {
		if (
			session.videoDevice === 1 &&
			(session.audioDevice === false || session.audioDevice == 1)
		) {
...
function addToGoogleCalendar() {
	...
	var details =
		"Join the live stream as a performer at the following link:<br/><br/>===>   " +
		linkout +
		"<br/><br/>To test your connection and camera ahead of time, please visit https://vdo.ninja/speedtest<br/><br/>Do not share the details of this invite with others, unless explicitly told to.";
...
function addToOutlookCalendar() {
	...
	var linkToOpen =
		"https://outlook.live.com/owa/?path=%2Fcalendar%2Faction%2Fcompose&rru=addevent&subject=" +
		title +
		"&body=" +
		details;
	//https://calendar.google.com/calendar/r/eventedit?text=My+Custom+Event&dates=20180512T230000Z/20180513T030000Z&details=For+details,+link+here:+https://example.com/tickets-43251101208&location=Garage+Boston+-+20+Linden+Street+-+Allston,+MA+02134
...
function addToYahooCalendar() {
	...
	details = details.split("&").join("%26");
	var linkToOpen =
		"https://calendar.yahoo.com?v60&title=" + title + "&desc=" + details;
	//https://calendar.google.com/calendar/r/eventedit?text=My+Custom+Event&dates=20180512T230000Z/20180513T030000Z&details=For+details,+link+here:+https://example.com/tickets-43251101208&location=Garage+Boston+-+20+Linden+Street+-+Allston,+MA+02134
...
function enumerateDevices() {
	...
	if (typeof navigator.enumerateDevices === "function") {
		...
		return new Promise((resolve, reject) => {
			try {
				if (
					window.MediaStreamTrack == null ||
					window.MediaStreamTrack.getSources == null
				) {
					throw new Error();
				}
		...
		return navigator.mediaDevices
			...
			.then(function (stream1) {
				...
				return enumerateDevices().then(function (deviceInfos) {
					...
					audioInputSelect.style.minHeight =
						(audioInputSelect.childElementCount + 1) * 1.15 * 16 + "px";
...
function gotDevices(deviceInfos) {
	// https://github.com/webrtc/samples/blob/gh-pages/src/content/devices/input-output/js/main.js#L19
	...
	try {
		...
		option.onchange = function (event) {
			...
			if (!getById("multiselect1").checked) {
				...
				while (
					SelectedAudioInputDevices.indexOf(event.currentTarget.value) > -1
				) {
					SelectedAudioInputDevices.splice(
						SelectedAudioInputDevices.indexOf(event.currentTarget.value),
						1
					);
				}
			}
		...
		for (let i = 0; i !== deviceInfos.length; ++i) {
			...
			if (
				!(
					deviceInfo.kind === "videoinput" &&
					(deviceInfo.label.toLowerCase().startsWith("ndi") ||
						deviceInfo.label.toLowerCase().startsWith("newtek"))
				)
			) {
				tmp.push(deviceInfo);
			}
		...
		for (let i = 0; i !== deviceInfos.length; ++i) {
			...
			if (
				...
				log(
					"V DEVICE FOUND = " +
						deviceInfo.label.replace(/[\W]+/g, "_").toLowerCase()
				);
			}
		}
		deviceInfos = tmp;
		...
		for (let i = 0; i !== deviceInfos.length; ++i) {
			...
			if (deviceInfo.kind === "audioinput") {
				...
				label.innerHTML =
					" " +
					(deviceInfo.label ||
						"microphone " + ((audioInputSelect.length || 0) + 1));
				...
				option.onchange = function (event) {
					...
					if (!CtrlPressed) {
						document
							.querySelectorAll("#audioSource input[type='checkbox']")
							.forEach(function (item) {
								if (event.currentTarget.id !== item.id) {
									...
									while (SelectedAudioInputDevices.indexOf(item.value) > -1) {
										SelectedAudioInputDevices.splice(
											SelectedAudioInputDevices.indexOf(item.value),
											1
										);
									}
								} else {
									item.checked = true;
									if (
										SelectedAudioInputDevices.indexOf(
									...
									if (
										session.mobile &&
										!(iOS || iPad) &&
										event.currentTarget.label === "USB audio" &&
										!session.cleanOutput
									) {
										warnUser(
											"Notice: USB audio devices may not work on all mobile devices.\n\nConsider using FireFox mobile instead, as it tends to work with USB audio devices more often."
										);
									}
								}
							});
				...
				/* 	listas.innerText = deviceInfo.label + ' - ' + deviceInfo.deviceId; */
				videoSelect.appendChild(option);
				option.setAttribute("selected", "");
				/* document.getElementById('sourcesList').appendChild(listas) */
...
function addScreenDevices(device) {
	if (device.kind == "audio") {
		...
		option.onchange = function (event) {
			...
			if (!CtrlPressed) {
				document
					.querySelectorAll("#audioSource3 input[type='checkbox']")
					.forEach(function (item) {
...
function gotDevices2(deviceInfos) {
	...
	try {
				...
				try {
					if (!knownTrack) {
						if (session.canvasSource) {
							session.canvasSource.srcObject
								.getVideoTracks()
								.forEach(function (track) {
		...
		if (videoSelect.childNodes.length <= 1) {
			...
			getById("flipcamerabutton").dataset.maxndex =
				videoSelect.childNodes.length;
		} else {
			getById("flipcamerabutton").style.display = "unset";
			getById("flipcamerabutton").dataset.maxIndex =
				videoSelect.childNodes.length;
		}
		...
		videoSelect.onchange = function (event) {
			...
			if (
				!getById("audioSource3").querySelectorAll("input[data-type='screen']")
					.length
			) {
				if (session.screenShareState) {
					session.screenShareState = false;
...
function gotDevicesRemote(deviceInfos, UUID) {
	try {
		if (document.getElementById("remoteVideoSelect_" + UUID)) {
			...
			videoSelect.style =
				"max-width:170px;font-size: 70% !important; margin: 5px 5px 5px 0; padding:2px;";
			videoSelect.onchange = function () {
				getById("requestVideoDevice_" + UUID).innerHTML =
					'<i class="las la-video"></i> request';
				getById("requestVideoDevice_" + UUID).title =
					"This will ask the remote guest for permission to change";
			};
		...
		if (document.getElementById("remoteAudioOutputSelect_" + UUID)) {
			var audioOutputSelect = document.getElementById(
				"remoteAudioOutputSelect_" + UUID
			);
		...
		for (let i = 0; i !== deviceInfos.length; ++i) {
			...
			if (deviceInfo.kind === "videoinput") {
				...
				option.value = deviceInfo.deviceId || "default";
				option.text =
					deviceInfo.label || `microphone ${audioOutputSelect.length + 1}`;
				if (
					getById("remoteAudioOutputSelect_" + UUID).innerText == option.text
				) {
					option.selected = "true";
				}
				audioOutputSelect.appendChild(option);
		...
		if (!matched) {
			...
			getById("requestVideoDevice_" + UUID).innerHTML =
				'<i class="las la-video"></i> refresh';
			getById("requestVideoDevice_" + UUID).title =
				"This will reconnect the guest's active video source.";
		}
	} catch (e) {
		errorlog(e);
	}
	...
	try {
		...
		if (mirror) {
			if (session.mirrored && session.flipped) {
				session.videoElement.style.transform =
					"scaleX(-1) scaleY(-1)" + transFlip;
		...
		if (document.getElementById("previewWebcam")) {
			var eleName = document.getElementById("previewWebcam");
			if (rotate) {
	...
	if (session.screenShareState == false) {
		...
		if (screenShareAudioTrack) {
			session.streamSrc.getAudioTracks().forEach(function (track) {
				...
				if (screenShareAudioTrack == track) {
...
if (navigator.userAgent.toLowerCase().indexOf(" electron/") > -1) {
	...
	try {
					...
					if (session.autostart) {
						if (parseInt(session.screenshare) + "" === session.screenshare) {
							...
							for (var i = 0; i < sources.length; i++) {
								if (sources[i].name.startsWith(session.screenshare)) {
							...
							if (sscid === null) {
								...
								for (var i = 0; i < sources.length; i++) {
									if (sources[i].name.includes(session.screenshare)) {
					  ...
					  `;
						...
						document
							.getElementById("cancelscreenshare")
							.addEventListener("click", async () => {
								selectionElem.remove();
								reject(null);
							});
						document
							.querySelectorAll(".desktop-capturer-click")
							.forEach((button) => {
		...
		warnlog(
			"Couldn't load electron's screen capture. Elevate the app's permission to allow it (right-click?)"
		);
	}
}
...
async function grabScreen(quality = 0, audio = true, videoOnEnd = false) {
	...
	if (navigator.userAgent.toLowerCase().indexOf(" electron/") > -1) {
		if (!ElectronDesktopCapture) {
			if (!session.cleanOutput) {
				warnUser(
					"Enable Elevated Privileges to allow screen-sharing. (right click this window to see that option)"
				);
			}
			return false;
		}
	...
	if (session.framerate) {
		...
		// not limiting screen share's fps with quality=2 due to gaming centric nature
		constraints.video.frameRate = {
			ideal: session.maxframerate,
			max: session.maxframerate,
		};
...
function toggleRoomSettings() {
	...
	if (getById("roomSettings").style.display == "none") {
		...
		document
			.getElementById("modalBackdrop")
			.addEventListener("click", toggleRoomSettings);
		document.getElementById("trbSettingInput").value = session.totalRoomBitrate;
		document.getElementById("trbSettingInputFeedback").innerHTML =
			session.totalRoomBitrate;
	}
}
...
function changeAudioDevice(index) {
	enumerateDevices()
		...
		.then(function () {
			activatedPreview = false;
			var audioSelect = document
				.getElementById("audioSource3")
...
function changeVideoDeviceById(deviceID, UUID = false) {
	enumerateDevices()
		...
		.then(function () {
			...
			if (index !== false) {
				if (document.getElementById("videoSource3").selectedIndex === j) {
					...
					window.focus();
					confirmAlt(
						"Allow the director to change your video device to:\n\n" +
							opts[index].text +
							" ?"
					).then((res) => {
						if (res) {
...
function changeAudioDeviceById(deviceID, UUID = false) {
	if (UUID && !session.consent) {
		window.focus();
		confirmAlt("Allow the director to change your audio mic source").then(
			(res) => {
				if (res) {
					enumerateDevices()
						.then(gotDevices2)
		...
		enumerateDevices()
			...
			.then(function () {
				...
				if (UUID && !session.consent) {
					window.focus();
					confirmAlt(
						"Allow the director to change your audio's speaker to:\n\n" +
							opts[index].text +
							" ?"
					).then((res) => {
						if (res) {
...
async function grabVideo(
	quality = 0,
	eleName = "previewWebcam",
	selector = "select#videoSourceSelect",
	callback = false
) {
	...
	session.videoElement.controls = session.showControls;
	...
	if (!videoSelect || videoSelect.value == "ZZZ") {
		...
		if (
			eleName == "previewWebcam" &&
			document.getElementById("previewWebcam")
		) {
			if (session.autostart) {
				...
				if (document.getElementById("gowebcam")) {
					...
					if (
						...
						document.getElementById("gowebcam").innerHTML =
							miscTranslations["start"];
					}
				}
			}
		} else {
		...
		if (session.facingMode) {
			...
			videoSelect.options[videoSelect.selectedIndex].text.includes("NDI Video")
		) {
		...
		if (session.forceAspectRatio) {
			if (constraints.video && constraints.video !== true) {
				constraints.video.aspectRatio = {
					ideal: parseFloat(session.forceAspectRatio),
				};
			}
		}
		...
		if (
			videoSelect.options[videoSelect.selectedIndex].text.startsWith(
				"OBS-Camera"
			)
		) {
			// OBS Virtualcam
			...
			videoSelect.options[videoSelect.selectedIndex].text.includes(" back")
		) {
			// Android
			mirror = true;
		} else if (
			videoSelect.options[videoSelect.selectedIndex].text.includes(" rear")
		) {
			// Android
			mirror = true;
		} else if (
			...
			if (session.quality !== 2 && !session.cleanOutput) {
				getById("cameraTipContext1").innerHTML =
					miscTranslations["camera-tip-c922"];
			...
			if (!session.cleanOutput) {
				getById("cameraTipContext1").innerHTML =
					miscTranslations["camera-tip-camlink"];
				getById("cameraTip1").classList.remove("advanced");
			}
		} else if (session.mobile) {
			mirrorcheck = true;
		...
		if (SamsungASeries && ChromeVersion) {
			if (!session.cleanOutput) {
				getById("cameraTipContext1").innerHTML =
					miscTranslations["samsung-a-series"];
				getById("cameraTip1").classList.remove("advanced");
			}
		}
			...
			function (gumID, callback2) {
				...
				navigator.mediaDevices
						...
						if (
							...
							if (session.autostart) {
								...
								if (document.getElementById("gowebcam")) {
									document.getElementById("gowebcam").dataset.ready = "true";
									if (
										document.getElementById("gowebcam").dataset.audioready ==
										"true"
									) {
										document.getElementById("gowebcam").disabled = false;
										document.getElementById("gowebcam").innerHTML =
					...
					.catch(function (e) {
						if (getUserMediaRequestID !== gumID) {
							warnlog(
								"the previously selected camera attempted failed, but not a big deal, since its now void"
							);
							return;
						}
						...
						if (e.name === "OverconstrainedError") {
							...
							if (quality <= 10) {
								...
								if (!session.cleanOutput) {
									if (iOS) {
										warnUser(
											"An error occured. Closing existing tabs in Safari may solve this issue."
										);
									} else {
										warnUser(
			...
			for (UUID in session.pcs) {
				try {
					...
					if (session.pcs[UUID].guest == true && session.roombitrate === 0) {
						log(
							"room rate restriction detected. No videos will be published to other guests"
						);
...
function senderAudioUpdate(callback = false) {
	try {
		...
		if (session.videoElement.srcObject.getAudioTracks()) {
			...
			if (session.mc && session.mc.getSenders) {
				session.mc.getSenders().forEach((sender) => {
					// disable senders that aren't part of the active tracks
					var good = false;
				...
				if (tracks.length) {
					tracks.forEach(function (track) {
						var matched = false;
						session.mc.getSenders().forEach((sender) => {
							// is the track in the current sender list?
							if (
								sender.track &&
								sender.track.id &&
								sender.track.kind == "audio"
						...
						if (matched) {
							// track already in the current sender list; skip
							return;
						}
						var added = false;
						session.mc.getSenders().forEach((sender) => {
							...
							if (
								...
								// replace instead of add new tracks; make sure to enable old tracks being replaced
						...
						var sender = session.mc.addTrack(
							track,
							session.videoElement.srcObject
						); //  didn't replace , since no old disabled tracks available, so let's add a new track.
					});
				} else {
					if (session.mc.getSenders) {
						session.mc.getSenders().forEach((sender) => {
							// disable all senders, since no tracks available.
				...
				if (session.pcs[UUID].allowAudio == true) {
					...
					if (tracks.length) {
						tracks.forEach(function (track) {
							...
							var sender = session.pcs[UUID].addTrack(
								track,
								session.videoElement.srcObject
							);
						});
	...
	if (document.getElementById("gowebcam")) {
		document.getElementById("gowebcam").dataset.audioready = true;
		if (
			document.getElementById("gowebcam").dataset.ready &&
			document.getElementById("gowebcam").dataset.ready == "true"
		) {
			document.getElementById("gowebcam").disabled = false;
...
// WEBCAM
session.publishDirector = async function (
	clean,
	vdevice = false,
	adevice = true
) {
	//  stream is used to generated an SDP ; true,false,false
	...
	try {
		...
		getById("webcamquality3").onchange = function (event) {
			if (
				parseInt(
					getById("webcamquality3").elements.namedItem("resolution").value
				) == 2
			) {
	...
	pokeIframeAPI("director-share", true);
...
session.publishStream = function (v) {
	...
	if (
		((session.roomid === false || session.roomid === "") &&
			session.quality === false) ||
		session.forceMediaSettings
	) {
		try {
...
async function publishScreen2(constraints, audioList = [], audio = true) {
	...
	for (var i = 1; i < audioList.length; i++) {
		...
		if (audioList[i].selected) {
			var constraint = {
				video: false,
				audio: { deviceId: { exact: audioList[i].value } },
			};
...
function addDownloadLink(fileList, UUID, pc) {
	...
	if (session.chat == false) {
		getById("chattoggle").className =
			"las la-comments my-float toggleSize puslate";
		getById("chatbutton").className = "float";
...
function updateDownloadLink(idx) {
	...
	if (elements[0]) {
		if (transferList[idx].status === 0) {
			...
			elements[0].innerHTML =
				"Downloading: " + parseInt(transferList[idx].completed * 100) + "%";
...
function fileShareMessage(fileinfo, idx) {
	...
	if (fileinfo.status === 0) {
		data.msg =
			" has a shared a file with you:<br /><i>" +
			fileinfo.name +
			"</i><br />Do you trust them? <button title='file size: " +
			fileinfo.size +
			" bytes' data-button-type='download' data-fid='" +
			fileinfo.id +
			"' data-tid='" +
			idx +
			"' onclick='requestFile(this);'>Download it here</button><button data-button-type='clear' data-fid='" +
			fileinfo.id +
			"' data-tid='" +
			idx +
			"' style='margin:10px 0 10px 2px;' onclick='clearDownloadFile(this);'>Clear</button>";
	} else if (fileinfo.status === 1) {
		data.msg =
			" has a shared a file with you:<br /><i>" +
			fileinfo.name +
			"</i><br /><button title='file size: " +
			fileinfo.size +
			" bytes' data-button-type='download' data-fid='" +
			fileinfo.id +
			"' data-tid='" +
		...
		data.msg =
			...
			"' data-tid='" +
			idx +
			"' onclick='cancelFile(this);'>Downloading: " +
			parseInt(transferList[idx].completed * 100) +
			"%</button>";
	} else if (fileinfo.status === 3) {
		data.msg =
			" has a shared a file with you:<br /><i>" +
			fileinfo.name +
			"</i><br /><button disabled title='file size: " +
			fileinfo.size +
			" bytes' data-button-type='download' data-fid='" +
			fileinfo.id +
			"' data-tid='" +
		...
		data.msg =
			...
			"' data-tid='" +
			idx +
			"' disabled >No longer available</button>";
...
function updateReshareLink() {
	...
	var shareLink =
		...
		added +
		pie;
	getById("reshare").href = shareLink;
	getById("reshare").text = shareLink;
	getById("reshare").style.width =
		(getById("reshare").text.length + 1) * 1.15 * 8 + "px";
...
session.publishFile = function (ele, event) {
	...
	function myHandler(e) {
		...
		vid.onloadeddata = function () {
			...
			toggleMute(true);
			session.streamSrc.getTracks().forEach(function (track) {
				// I'm making an exception I guess -- reversing the role?
				for (UUID in session.pcs) {
					if ("realUUID" in session.pcs[UUID]) {
						continue;
					}
...
function dragElement(elmnt) {
	...
	function dragMouseDown(e) {
		...
		//elmnt.controls = session.showControls;
		e = e || window.event;
		e.preventDefault();
...
function previewIframe(iframeSrc) {
	...
	var iframe = document.createElement("iframe");
	iframe.allow =
		"autoplay;camera;microphone;fullscreen;picture-in-picture;display-capture;";
...
function checkIfPIP() {
	try {
		if (
			session.videoElement &&
			((session.videoElement.webkitSupportsPresentationMode &&
				typeof session.videoElement.webkitSetPresentationMode === "function") ||
			...
			getById("pIpStartButton").addEventListener("click", function (event) {
				...
				//	} else {
				session.videoElement.webkitSetPresentationMode(
					session.videoElement.webkitPresentationMode === "picture-in-picture"
						? "inline"
						: "picture-in-picture"
				);
				//	}
...
function listAudioSettingsPrep() {
	...
	for (var i = 0; i < tracks.length; i += 1) {
		...
		for (var waid in session.webAudios) {
			...
			try {
				if (
					session.webAudios[waid].subGainNodes &&
					track0.id in session.webAudios[waid].subGainNodes
				) {
					trackSet.subGain =
						session.webAudios[waid].subGainNodes[track0.id].gain.value;
				}
				break;
			} catch (e) {}
		}
...
async function requestVideoRecord(ele) {
	...
	if (ele.classList.contains("pressed")) {
		...
		msg.UUID = UUID;
		window.focus();
		var bitrate = await promptAlt(
			miscTranslations["what-bitrate"],
			false,
			false,
			6000
		);
...
function changeOrderDirector(value) {
	...
	var elements = document.querySelectorAll(
		'[data-action-type="order-value-director"]'
	);
	//log(elements);
	if (elements[0]) {
...
function requestVideoHack(keyname, value, UUID, ctrl = false) {
	...
	session.sendRequest(msg, msg.UUID);
	pokeIframeAPI(
		"request-video-setting",
		{ value: value, keyname: keyname, ctrl: ctrl },
		UUID
	);
}
...
function requestAudioHack(keyname, value, UUID, deviceID = "default") {
	...
	session.sendRequest(msg, msg.UUID);
	pokeIframeAPI(
		"request-audio-setting",
		{ value: value, keyname: keyname, deviceID: deviceID },
		UUID
	);
}
...
function requestChangeEQ(keyname, value, UUID, track = 0) {
	...
	pokeIframeAPI(
		"request-change-eq",
		{ value: value, keyname: keyname, track: track },
		UUID
	);
}
...
function toggleSystemPip(vid) {
	if (
		vid.webkitSupportsPresentationMode &&
	...
	for (var n = 0; n < dataN.length; n += 1) {
		...
		if (data.lowcut !== false && n == 0) {
			...
			input.onchange = function (e) {
				...
				getById("label_" + e.target.dataset.keyname).innerText =
					e.target.dataset.labelname + " " + e.target.value;
				requestChangeLowcut(
					parseInt(e.target.value),
					e.target.dataset.UUID,
					parseInt(e.target.dataset.track)
				);
			};
		...
		for (var i in data.audioConstraints) {
			try {
				...
				if (
					typeof data.audioConstraints[i] === "object" &&
					data.audioConstraints[i] !== null &&
					"max" in data.audioConstraints[i] &&
					"min" in data.audioConstraints[i]
				) {
					if (i === "aspectRatio") {
						continue;
					} else if (i === "width") {
					...
					if (!("deviceID" in data.audioConstraints[i])) {
						continue;
					...
					if (i === "height" || i === "width") {
						input.title =
							"Hold CTRL (or cmd) to lock width and height together when changing them";
						input.min = 16;
					}
					...
					input.onchange = function (e) {
						//e.target.title = e.target.value;
						getById(
							"label_" + e.target.dataset.keyname + "_" + e.target.dataset.track
						).innerText = e.target.dataset.keyname + ": " + e.target.value;
						//updateAudioConstraints(e.target.dataset.keyname, e.target.value);
						requestAudioHack(
							e.target.dataset.keyname,
							e.target.value,
							e.target.dataset.UUID,
					...
					if (data.audioConstraints[i].length > 1) {
						for (var opts in data.audioConstraints[i]) {
							...
							if (i in data.currentAudioConstraints) {
								if (
									data.audioConstraints[i][opts] ==
									data.currentAudioConstraints[i]
								) {
									opt.selected = "true";
								}
							}
...
function updateDirectorsVideo(data, UUID) {
	...
	for (var i in data.cameraConstraints) {
		try {
			...
			if (
				typeof data.cameraConstraints[i] === "object" &&
				data.cameraConstraints[i] !== null &&
				"max" in data.cameraConstraints[i] &&
				"min" in data.cameraConstraints[i]
			) {
				if (i === "aspectRatio") {
					continue;
				} else if (i === "width") {
				...
				input.onchange = function (e) {
					getById("label_" + e.target.dataset.keyname).innerText =
						e.target.dataset.keyname + ": " + e.target.value;
				...
				if (data.cameraConstraints[i].length > 1) {
					for (var opts in data.cameraConstraints[i]) {
						...
						if (i in data.currentCameraConstraints) {
							if (
								data.cameraConstraints[i][opts] ==
								data.currentCameraConstraints[i]
							) {
								opt.selected = "true";
							}
						}
	...
	for (var ii = 0; ii < tracks.length; ii++) {
		...
		if (session.lowcut && ii == 0) {
			...
			for (var webAudio in session.webAudios) {
				if (session.webAudios[webAudio].lowcut1.frequency) {
					...
					label.innerHTML +=
						" " + session.webAudios[webAudio].lowcut1.frequency.value;
			...
			input.onchange = function (e) {
				getById("label_" + e.target.dataset.keyname).innerHTML =
					e.target.dataset.labelname + " " + e.target.value;
		...
		if (session.equalizer && ii == 0) {
			...
			for (var webAudio in session.webAudios) {
				if (session.webAudios[webAudio].highEQ.gain) {
					...
					label.innerHTML +=
						" " + session.webAudios[webAudio].highEQ.gain.value;
					input.title = input.value;
				}
			}
		...
		for (var i in session.audioConstraints) {
			try {
				...
				if (
					typeof session.audioConstraints[i] === "object" &&
					session.audioConstraints[i] !== null &&
					"max" in session.audioConstraints[i] &&
					"min" in session.audioConstraints[i]
				) {
					if (i === "aspectRatio") {
						continue;
					} else if (i === "width") {
					...
					if (i in session.currentAudioConstraints) {
						...
						label.title =
							"Previously was:  " + session.currentAudioConstraints[i];
						input.title =
							"Previously was:  " + session.currentAudioConstraints[i];
					} else {
						label.innerHTML = i;
						...
						try {
							getById(
								"label_" +
									...
									"_" +
									e.target.dataset.track
							).innerHTML = e.target.dataset.keyname + ": " + e.target.value;
							applyAudioHack(
								e.target.dataset.keyname,
								e.target.value,
								e.target.dataset.deviceid
					...
					label.innerHTML = i + ":";
					label.style =
						"display:inline-block; padding:0;margin: 15px 0px 29px;";
					label.dataset.keyname = i;
					...
					if (session.audioConstraints[i].length > 1) {
						for (var opts in session.audioConstraints[i]) {
							log(opts);
							var opt = new Option(
								session.audioConstraints[i][opts],
								session.audioConstraints[i][opts]
							);
							input.options.add(opt);
							...
							if (i in session.currentAudioConstraints) {
								if (
									session.audioConstraints[i][opts] ==
									session.currentAudioConstraints[i]
								) {
									opt.selected = "true";
								}
							}
		...
		if (tracks.length > 1) {
			for (var webAudio in session.webAudios) {
				if (
					session.webAudios[webAudio].subGainNodes &&
					track0.id in session.webAudios[webAudio].subGainNodes
				) {
					if (
						getById("popupSelector_constraints_audio").style.display == "none"
					) {
					...
					input.value =
						session.webAudios[webAudio].subGainNodes[track0.id].gain.value *
						100;
					label.innerHTML +=
						" " +
						parseInt(
			...
			for (var webAudio in session.webAudios) {
				if (session.webAudios[webAudio].gainNode) {
					...
					label.innerHTML +=
						" " +
						parseInt(session.webAudios[webAudio].gainNode.gain.value * 100);
					input.title = parseInt(input.value);
...
function listCameraSettings() {
	...
	if (
		...
		label.innerHTML = i + ":";
		label.title =
			"If you're on a slow network, you can improve frame rate and audio quality by reducing the amount of video data that others send you";
	...
	for (var i in session.cameraConstraints) {
		try {
			...
			if (
				typeof session.cameraConstraints[i] === "object" &&
				session.cameraConstraints[i] !== null &&
				"max" in session.cameraConstraints[i] &&
				"min" in session.cameraConstraints[i]
			) {
				if (i === "aspectRatio") {
					continue;
				} else if (i === "frameRate") {
				...
				if (i in session.currentCameraConstraints) {
					...
					label.title =
						"Previously was:  " + session.currentCameraConstraints[i];
					input.title =
						"Previously was:  " + session.currentCameraConstraints[i];
				} else {
					label.innerHTML = i;
				...
				if (session.cameraConstraints[i].length > 1) {
					...
					for (var opts in session.cameraConstraints[i]) {
						log(opts);
						var opt = new Option(
							session.cameraConstraints[i][opts],
							session.cameraConstraints[i][opts]
						);
						input.options.add(opt);
						if (i in session.currentCameraConstraints) {
							if (
								session.cameraConstraints[i][opts] ==
								session.currentCameraConstraints[i]
							) {
								opt.selected = "true";
								included = true;
							}
					...
					session.cameraConstraints[i].length &&
					"continuous" == session.cameraConstraints[i][0]
				) {
					...
					session.cameraConstraints[i].length &&
					"manual" == session.cameraConstraints[i][0]
				) {
					var opt = new Option("manual", "manual");
					input.options.add(opt);
					if (i in session.currentCameraConstraints) {
					...
					if (cameraSettings["current"]) {
						for (var i in cameraSettings["default"]) {
							...
							if (i in cameraSettings["current"]) {
								if (
									cameraSettings["current"][i] != cameraSettings["default"][i]
								) {
									if (i in session.cameraConstraints) {
										if ("min" in session.cameraConstraints[i]) {
											if (
												session.cameraConstraints[i].min >
												cameraSettings["default"][i]
											) {
												continue;
											}
										}
										if ("max" in session.cameraConstraints[i]) {
											if (
												session.cameraConstraints[i].max <
												cameraSettings["default"][i]
											) {
												continue;
											}
...
function updateSavedVideoSettings(track0) {
...
function updateCameraConstraints(
	...
	try {
		if (track0.getSettings) {
			...
			if (session.currentCameraConstraints.deviceId) {
				if (
					!getStorage("camera_" + session.currentCameraConstraints.deviceId)
				) {
					cameraSettings["default"] = JSON.parse(
						JSON.stringify(session.currentCameraConstraints)
					);
					log(cameraSettings["default"]);
				} else {
					cameraSettings = getStorage(
						"camera_" + session.currentCameraConstraints.deviceId
					);
				}
			}
		}
	...
	if (
		!ctrl &&
		constraint == "width" &&
		"height" in session.currentCameraConstraints
	) {
		var constraits = {
			width: value,
			height: session.currentCameraConstraints.height,
		};
	} else if (
		!ctrl &&
		constraint == "height" &&
		"width" in session.currentCameraConstraints
	) {
		var constraits = {
			height: value,
			width: session.currentCameraConstraints.width,
		};
	} else {
...
function setupWebcamSelection(stream = null) {
	...
	try {
		return enumerateDevices()
			...
			.then(function () {
				if (
					getById("webcamquality").elements &&
					parseInt(
						getById("webcamquality").elements.namedItem("resolution").value
					) == 3
				) {
					if (session.maxframerate === false) {
						session.maxframerate = 30;
				...
				audioSelect.onchange = function () {
					if (document.getElementById("gowebcam")) {
						...
						document.getElementById("gowebcam").style.backgroundColor =
							"#DDDDDD";
						document.getElementById("gowebcam").style.fontWeight = "normal";
						document.getElementById("gowebcam").innerHTML =
							"Waiting for mic to load";
						miniTranslate(
							document.getElementById("gowebcam"),
							"waiting-for-mic-to-load"
						);
					}
					activatedPreview = false;
				...
				if (
					document.getElementById("previewWebcam") &&
					document.getElementById("previewWebcam").setSinkId
				) {
					if (session.sink) {
						getById("previewWebcam")
							.setSinkId(session.sink)
...
async function shareWebsite(autostart = false, evt = false) {
	if (session.iframeSrc) {
		if (evt && (evt.ctrlKey || evt.metaKey)) {
			if (getById("websitesharebutton2").classList.contains("green")) {
				...
				getById("websitesharebutton2").title =
					"Hold CTRL (or CMD) and click to spotlight this shared website";
			} else {
				if (session.streamID) {
					var actionMsg = {};
					...
					getById("websitesharebutton2").title =
						"Video is currently spotlighted";
				}
			}
			return;
		}
		...
		getById("websitesharebutton2").classList.add("advanced");
		getById("websitesharebutton").classList.remove("advanced");
	...
	if (autostart === false) {
		window.focus();
		var iframeURL = await promptAlt(
			miscTranslations["enter-website"],
			false,
			false,
...
function createIframePopup() {
	...
	if (
		(session.queue && !session.transferred) ||
		(session.screenShareState && !session.queue && session.transferred)
	) {
	...
	if (session.screenshareid) {
		...
		var iFrameID =
			session.streamID.substring(0, 12) + "_" + session.generateStreamID(5);
	}
	...
	if (session.muted) {
		iframe.src =
			"./?audiodevice=1&screenshare&transparent&cleanish&noheader&autostart&view&muted&room=" +
			session.roomid +
			"&push=" +
			iFrameID +
			extras;
		...
		iframe.src =
			"./?audiodevice=1&screenshare&transparent&cleanish&noheader&autostart&view&room=" +
			session.roomid +
			"&push=" +
			iFrameID +
			extras;
	...
	if (session.audioDevice === 0) {
		...
		if (session.echoCancellation !== false) {
			...
			if (!session.cleanoutput) {
				...
				getById("headphoneTipContext1").innerHTML =
					miscTranslations["headphones-tip"];
			}
		}
		if (session.autoGainControl !== false) {
			constraint.audio.autoGainControl = true;
...
function previewWebcam2() {
	...
	document.getElementById("loader-preview").classList.remove("hide");
	document.getElementById("loader-preview").classList.add("show");
}
					...
					if (!session.cleanOutput) {
						setTimeout(function () {
							if (window.obsstudio) {
								...
								warnUser(
									"Permissions denied. Please ensure you have allowed the mic/camera permissions."
								);
							}
						}, 1);
					}
...
async function updateLinkWelcome(arg, input) {
	if (input.checked) {
		var response = await promptAlt(
			"Enter the message you'd like the guests to see"
		);
...
function updateLinkWebP(arg, input) {
	if (input.checked) {
		if (
			!(
				getById("director_block_" + arg).dataset.raw.includes("&broadcast") ||
				getById("director_block_" + arg).dataset.raw.includes("?broadcast")
			)
		) {
			getById("broadcastSlider").checked = true;
...
function changeURL(changeURL) {
	window.focus();
	confirmAlt(
		miscTranslations["director-redirect-1"] +
			changeURL +
			miscTranslations["director-redirect-2"]
	).then((res) => {
		if (res) {
			hangup();
...
function generateQRPageCallback(hash) {
	try {
		...
		if (getById("invite_password").value.trim().length) {
			...
			viewstr +=
				"&password=" +
				sanitizePassword(getById("invite_password").value.trim());
		}
...
function initSceneList(UUID) {
	Object.keys(session.sceneList).forEach((scene, index) => {
		if (
			getById("container_" + UUID).querySelectorAll(
				'[data-scene="' + scene + '"]'
			).length
		) {
			return;
...
function unPauseVideo(videoEle, update = true) {
	try {
		...
		session.rpcs[videoEle.dataset.UUID].manualBandwidth =
			session.rpcs[videoEle.dataset.UUID].prePausedBandwidth;
		...
		videoEle.classList.remove("fadeout");
		if (update) {
			updateMixer();
		}
	} catch (e) {
...
function pauseVideo(videoEle, update = true) {
	...
	videoEle.classList.add("fadeout");
	if (update) {
		updateMixer();
	}
}
...
(function rightclickmenuthing() {
	...
	async function menuItemListener(link) {
		if (link.getAttribute("data-action") === "Open") {
			...
			if (
				taskItemInContext.id == "videosource" ||
				taskItemInContext.id == "previewWebcam"
			) {
			...
			//copyFunction(taskItemInContext.href);
			var response = await promptAlt(
				"Please note, manual edits to the URL may conflict with the toggles",
				false,
				false,
		...
		for (var i = 0; i < items.length; i++) {
			if (items[i].getAttribute("data-action") === "FullWindow") {
				if (
					...
					if (session.infocus === true) {
						items[i].parentNode.classList.add("advanced");
					} else {
						items[i].parentNode.classList.remove("advanced");
					}
				...
				if (taskItemInContext.dataset.UUID in session.rpcs) {
					if (
						"prePausedBandwidth" in session.rpcs[taskItemInContext.dataset.UUID]
					) {
						items[i].parentNode.classList.add("advanced");
					} else {
						items[i].parentNode.classList.remove("advanced");
...
var messageList = [];
function sendChatMessage(chatMsg = false) {
	// filtered + visual
	var data = {};
	if (chatMsg === false) {
	...
	if (session.label) {
		if (session.director) {
			label =
				"<b><i><span class='chat_name'>" + session.label + "</span>:</i></b> ";
		} else {
	...
	if (msg.trim() === "/list") {
		...
		for (var UUID in session.rpcs) {
			if (UUID.startsWith(uid)) {
				...
				session.rpcs[UUID].label &&
				session.rpcs[UUID].label.toLowerCase().startsWith(uid)
			) {
				sendChat(msg, UUID); // send message to peers
				var data = {};
				data.time = Date.now();
				...
				session.directorList.indexOf(UUID) >= 0 &&
				"director".startsWith(uid)
			) {
				sendChat(msg, UUID); // send message to peers
				var data = {};
				data.time = Date.now();
		...
		for (var UUID in session.pcs) {
			...
			if (UUID.startsWith(uid)) {
				...
				session.pcs[UUID].label &&
				session.pcs[UUID].label.toLowerCase().startsWith(uid)
			) {
				sendChat(msg, UUID); // send message to peers
				var data = {};
				data.time = Date.now();
...
function setClock(initial = false) {
	...
	getById("overlayClock").innerHTML =
		zpadTime(minutes) + ":" + zpadTime(seconds);
...
function pauseClock() {
	clearInterval(clockOverlayTimer);
	var current =
		Date.now() - parseInt(getById("overlayClockContainer").dataset.timestamp);
	current =
		parseInt(getById("overlayClockContainer").dataset.initial) -
		parseInt(Math.round(current / 1000));
...
function getChatMessage(msg, label = false, director = false, overlay = false) {
	...
	if (label) {
		...
		if (director) {
			data.label =
				"<b><i><span class='chat_director chat_name'>" +
				data.label +
				"</span>:</i></b> ";
		} else {
		...
		data.label =
			"<b><i><span class='chat_director chat_name'>Director</span>:</i></b> ";
...
function updateClosedCaptions(msg, label, UUID) {
	...
	if (textOverlay) {
		...
		spanOverlay.style.fontSize =
			(parseInt(session.labelsize || 100) / 100.0) * 4.5 + "vh";
		spanOverlay.style.lineHeight =
			(parseInt(session.labelsize || 100) / 100) * 6 + "vh";
		spanOverlay.style.margin =
			(parseInt(session.labelsize || 100) / 100.0) * 0.75 + "vh";
...
function updateMessages() {
	...
	for (var i in messageList) {
		...
		if (messageList[i].type == "sent") {
			...
			messageList[i].type == "recv" ||
			messageList[i].type == "action"
		) {
			var label = "";
	...
	for (var i in msgTransferList) {
		...
		if (msgTransferList[i].type == "sent") {
			msg.innerHTML =
				msgTransferList[i].msg +
				" <i><small> <small>- " +
				time +
				"</small></small></i>";
			msg.classList.add("outMessage");
		} else if (
			msgTransferList[i].type == "recv" ||
			msgTransferList[i].type == "action"
		) {
			var label = "";
...
function showCustomizer(arg, ele) {
	...
	if (getById("customizeLinks" + arg).style.display != "none") {
		...
		getById("showCustomizerButton" + arg).style.boxShadow =
			"inset 0px 0px 1px #b90000";
...
async function recordVideo(target, event, videoKbps = false) {
	...
	if ("recording" in video) {
		...
		var elements = document.querySelectorAll(
			'[data-action-type="change-quality2"][data--u-u-i-d="' + UUID + '"]'
		);
		if (elements[0]) {
			elements[0].classList.add("pressed");
		}
		var elements = document.querySelectorAll(
			'[data-action-type="change-quality1"][data--u-u-i-d="' + UUID + '"]'
		);
		if (elements[0]) {
			elements[0].classList.remove("pressed");
		}
		var elements = document.querySelectorAll(
			'[data-action-type="change-quality3"][data--u-u-i-d="' + UUID + '"]'
		);
		if (elements[0]) {
			elements[0].classList.remove("pressed");
		}
	...
	if (videoKbps == false) {
		if (defaultRecordingBitrate == false) {
			...
			window.focus();
			videoKbps = await promptAlt(
				miscTranslations["press-ok-to-record"],
				false,
				false,
				videoKbps
			);
	...
	if (videoKbps) {
		...
		if (
			tryCodec &&
			MediaRecorder.isTypeSupported("video/webm;codecs=" + tryCodec)
		) {
			if (!session.cleanOutput) {
			...
			if (session.pcm) {
				if (
					MediaRecorder.isTypeSupported(
						'video/webm;codecs="' + tryCodec + ', pcm"'
					)
				) {
	...
	function handleDataAvailable(event) {
		if (event.data && event.data.size > 0) {
			...
			try {
				...
				if (video.recording) {
					updateLocalRecordButton(
						UUID,
						parseInt((Date.now() - timestamp) / 1000) || 0
					);
				}
			} catch (e) {
	...
	video.recorder.mediaRecorder.onerror = function (event) {
		...
		if (!session.cleanOutput) {
			setTimeout(function () {
				warnUser(
					"an error occured with the media recorder; stopping recording"
				);
			}, 1);
		}
	};
...
function updateRemoteRecordButton(UUID, recorder) {
	var elements = document.querySelectorAll(
		'[data-action-type="recorder-remote"][data--u-u-i-d="' + UUID + '"]'
	);
	if (elements[0]) {
		...
		if (time == -4) {
			if (!session.cleanOutput) {
				warnUser(
					"A remote recording has stopped unexpectedly.\n\nDid a user cancel the file downlaod?"
				);
			}
			if (session.beepToNotify) {
				playtone();
			...
			if (!session.cleanOutput) {
				setTimeout(function () {
					warnUser(
						"The remote browser does not support recording.\n\nPerhaps try local recording instead."
					);
				}, 0);
			}
		} else if (time == -2) {
			...
			elements[0].classList.remove("pressed");
			elements[0].innerHTML =
				'<i class="las la-circle"></i> <span data-translate="record-remote"> Record Remote</span>';
		} else {
			var minutes = Math.floor(time / 60);
			var seconds = time - minutes * 60;
			elements[0].classList.add("pressed");
...
function updateLocalRecordButton(UUID, recorder) {
	var elements = document.querySelectorAll(
		'[data-action-type="recorder-local"][data--u-u-i-d="' + UUID + '"]'
	);
	if (elements[0]) {
		var time = parseInt(recorder) || 0;
		...
		if (time == -3) {
			elements[0].classList.add("pressed");
			elements[0].innerHTML =
				'<i class="las la-check"></i> <span data-translate="record"> ARMED</span>';
			...
			elements[0].classList.remove("pressed");
			elements[0].innerHTML =
				'<i class="las la-circle"></i> <span data-translate="record-local"> Record Local</span>';
			elements[0].style.backgroundColor = "";
		} else {
			var minutes = Math.floor(time / 60);
			var seconds = time - minutes * 60;
...
function recordLocalVideoToggle() {
	...
	if (ele.dataset.state == "0") {
		...
		if (session.director) {
			var elements = document.querySelectorAll(
				'[data-action-type="recorder-local"][data-sid="' +
					session.streamID +
					'"]'
			);
			if (elements[0]) {
				elements[0].classList.add("pressed");
				elements[0].innerHTML =
					'<i class="las la-stop-circle"></i><span data-translate="record-local"> Record</span>';
			}
		}
	} else {
		if ("recording" in session.videoElement) {
		...
		if (session.director) {
			...
			if (elements[0]) {
				elements[0].classList.remove("pressed");
				elements[0].innerHTML =
					'<i class="las la-circle"></i><span data-translate="record-local"> Record</span>';
			}
		}
	}
}
...
function setupSensorData(pollrate = 30) {
	...
	if (window.Accelerometer) {
		...
		session.sensors.Accelerometer.addEventListener("reading", (e) => {
			...
			session.sensors.data.acc.t = parseInt(
				Math.round(session.sensors.Accelerometer.timestamp)
			);
		});
	...
	if (window.Gyroscope) {
		...
		session.sensors.Gyroscope.addEventListener("reading", (e) => {
			...
			session.sensors.data.gyro.t = parseInt(
				Math.round(session.sensors.Gyroscope.timestamp)
			);
		});
	...
	if (window.Magnetometer) {
		...
		session.sensors.Magnetometer.addEventListener("reading", (e) => {
			...
			session.sensors.data.mag.t = parseInt(
				Math.round(session.sensors.Magnetometer.timestamp)
			);
		});
		...
		try {
			window.addEventListener("deviceorientation", (e) => {
				...
				session.sensors.data.ori.t =
					parseInt(Math.round(e.timestamp)) || Date.now();
			});
	...
	if (window.LinearAccelerationSensor) {
		...
		session.sensors.LinearAccelerationSensor.addEventListener(
			"reading",
			(e) => {
				...
				session.sensors.data.lin.t = parseInt(
					Math.round(session.sensors.LinearAccelerationSensor.timestamp)
				);
			}
		);
...
function recordLocalVideo(action = null, videoKbps = 6000, remote = false) {
	// event.currentTarget,this.parentNode.parentNode.dataset.UUID
	var audioKbps = false;
	if (remote) {
		var video = remote;
	...
	if ("recording" in video) {
		if (action == "stop") {
			...
			getById("recordLocalbutton").innerHTML =
				'<i class="toggleSize my-float las la-square" ></i>';
			return;
		} else {
		...
		if (!remote) {
			if (restart) {
				if (getById("recordLocalbutton").dataset.state == 2) {
					...
					getById("recordLocalbutton").innerHTML =
						'<i class="toggleSize my-float las la-exclamation" ></i>';
					restart = false;
					warnUser("Media Recording Stopped due to an error.");
				} else {
					getById("recordLocalbutton").innerHTML =
						'<i class="toggleSize my-float las la-spinner" ></i>';
				...
				getById("recordLocalbutton").style.backgroundColor = "";
				getById("recordLocalbutton").innerHTML =
					'<i class="toggleSize my-float las la-dot-circle" ></i>';
				if (notify) {
					if (!session.cleanOutput) {
...
function changeAudioOutputDevice(ele) {
	try {
		if (session.sink) {
			if (iOS || iPad) {
				return;
			} // iOS devices do not support this.
			...
			if (typeof ele.sinkId !== "undefined") {
				navigator.mediaDevices
					...
					.catch(function canplayspecificaudio() {
						errorlog(
							"Can't play out to specific audio device without mic permissions allowed"
						);
					});
			} else {
			...
			if (!added) {
				...
				if (trk.muted && trk.kind == "video" && session.director) {
					...
					if (session.rpcs[UUID].videoElement.controls) {
						session.rpcs[UUID].videoElement.controls = session.showControls;
						setTimeout(
							function (ele) {
								if (ele) {
									ele.controls = true;
		...
		if (session.audioEffects === true) {
			...
			tracks.forEach((trk) => {
				...
				if (!added) {
					...
					warnlog(
						"this shouldn't happen that often, audio track orphaned. removing it"
					);
...
function addAudioPipeline(UUID, track) {
	...
	try {
		...
		session.rpcs[UUID].inboundAudioPipeline[trackid].mediaStream =
			createMediaStream();
		session.rpcs[UUID].inboundAudioPipeline[trackid].mediaStream.addTrack(
			track
		);
		...
		session.rpcs[UUID].inboundAudioPipeline[trackid].mutedAudio.srcObject =
			session.rpcs[UUID].inboundAudioPipeline[trackid].mediaStream; // needs to be added as an streamed element to be usable, even if its hidden
		...
		source = session.audioCtx.createMediaStreamSource(
			session.rpcs[UUID].inboundAudioPipeline[trackid].mediaStream
		);
		...
		if (session.rpcs[UUID].channelOffset !== false) {
			...
			session.rpcs[UUID].inboundAudioPipeline[trackid].destination =
				session.audioCtx.createMediaStreamDestination();
			source = offsetChannel(
				session.rpcs[UUID].inboundAudioPipeline[trackid].destination,
				source,
				session.rpcs[UUID].channelOffset
			);
			screwedUp = true;
		...
		if (screwedUp) {
			...
			return session.rpcs[UUID].inboundAudioPipeline[
				trackid
...
function changeGroup(ele, state = null) {
	...
	if (session.rpcs[ele.dataset.UUID].group.length) {
		session.sendRequest(
			{ group: session.rpcs[ele.dataset.UUID].group.join(",") },
			ele.dataset.UUID
		);
	} else {
...
function changeChannelOffset(UUID, channel) {
	var ele = document.querySelectorAll(
		'[data-action-type="add-channel"][data--u-u-i-d="' + UUID + '"]'
	);
	...
	for (var trackid in session.rpcs[UUID].inboundAudioPipeline) {
		if ("panNode" in session.rpcs[UUID].inboundAudioPipeline[trackid]) {
			session.rpcs[UUID].inboundAudioPipeline[
				trackid
			].panNode.pan.setValueAtTime(value, session.audioCtx.currentTime);
		}
		if ("gainPanNode" in session.rpcs[UUID].inboundAudioPipeline[trackid]) {
			session.rpcs[UUID].inboundAudioPipeline[
				trackid
			].gainPanNode.setValueAtTime(
				1 - Math.abs(value) / 2,
...
function addDelayNode(source, UUID, trackid) {
	...
	session.rpcs[UUID].inboundAudioPipeline[trackid].delayNode =
		session.audioCtx.createDelay(delay + 5); // 5 seconds additionally added for the purpose of flexibility

	session.rpcs[UUID].inboundAudioPipeline[trackid].delayNode.delayTime.value =
		delay; // delayTime takes it in seconds.
...
function applyStyleEffect(UUID) {
	...
	if (session.style == 3) {
		...
		session.rpcs[UUID].canvasCtx.fillRect(
			0,
			0,
			session.rpcs[UUID].canvas.width,
			session.rpcs[UUID].canvas.height
		);
		...
		session.rpcs[UUID].canvasCtx.fillStyle =
			"rgb(" + r + ", " + g + ", " + b + ")";
		session.rpcs[UUID].canvasCtx.fillRect(
			0,
			0,
		...
		session.rpcs[UUID].canvasCtx.arc(
			parseInt(session.rpcs[UUID].canvas.width / 2),
			parseInt(session.rpcs[UUID].canvas.height / 2),
			parseInt(session.rpcs[UUID].canvas.height / 4),
			0,
			2 * Math.PI,
			false
		);
		...
		if (session.rpcs[UUID].label) {
			...
			session.rpcs[UUID].canvasCtx.font =
				parseInt(session.rpcs[UUID].canvas.height / 2.11) + "px Arial";
			session.rpcs[UUID].canvasCtx.fillText(
				session.rpcs[UUID].label[0].toUpperCase(),
				parseInt(session.rpcs[UUID].canvas.width / 2),
				parseInt((session.rpcs[UUID].canvas.height * 2) / 3)
			);
		} else {
			var tmp = getComputedStyle(document.querySelector(":root"))
				.getPropertyValue("--video-background-image")
				.split('"');
			if (tmp.length === 3) {
				var img = new Image();
				img.onload = function () {
					session.rpcs[UUID].canvasCtx.fillStyle = "rgb(25,0,0)";
					session.rpcs[UUID].canvasCtx.drawImage(
						img,
						parseInt(session.rpcs[UUID].canvas.width / 2 - 110),
						parseInt(session.rpcs[UUID].canvas.height / 2 - 110),
						220,
						220
					);
				};
...
function fftWaveform(source, UUID, trackid) {
	...
	session.rpcs[UUID].inboundAudioPipeline[trackid].analyser =
		session.audioCtx.createAnalyser();
	session.rpcs[UUID].inboundAudioPipeline[trackid].analyser.fftSize = 512;
	var bufferLength =
		session.rpcs[UUID].inboundAudioPipeline[trackid].analyser.frequencyBinCount;
	var dataArray = new Uint8Array(bufferLength);
	session.rpcs[UUID].inboundAudioPipeline[
		trackid
		...
		function (uuid) {
			try {
				...
				if (session.pushLoudness == true) {
					var loudnessObj = {};
					loudnessObj[session.rpcs[uuid].streamID] =
						session.rpcs[uuid].stats.Audio_Loudness;
				...
				session.rpcs[uuid].canvasCtx.lineTo(
					session.rpcs[uuid].canvas.width,
					session.rpcs[uuid].canvas.height / 2
				);
...
function audioMeterGuest(mediaStreamSource, UUID, trackid) {
	...
	function updateLevels() {
		...
		try {
					...
					parent.postMessage(
						{
							loudness: loudnessObj,
							action: "loudness",
							value: session.rpcs[UUID].stats.Audio_Loudness,
							UUID: UUID,
						},
						"*"
					);
			...
			try {
				clearTimeout(
					session.rpcs[UUID].inboundAudioPipeline[trackid].analyser.interval
				);
				session.rpcs[UUID].inboundAudioPipeline[trackid].analyser.interval =
					setTimeout(function () {
						updateLevels();
			...
			if (session.rpcs[UUID].voiceMeter) {
				...
				if (session.meterStyle == 1) {
					session.rpcs[UUID].voiceMeter.style.height =
						Math.min(total, 100) + "%";
					if (total > 75) {
						total = Math.min(100, (total - 75) * 4);
						var R = parseInt((255 * total) / 100)
							.toString(16)
							.padStart(2, "0");
						var G = parseInt((255 * (100 - total)) / 100)
							.toString(16)
							.padStart(2, "0");
						session.rpcs[UUID].voiceMeter.style.backgroundColor =
							"#" + R + G + "00";
					} else {
...
function effectsDynamicallyUpdate(event, ele) {
	...
	if (session.effects == "0" || !session.effects) {
		...
		if (!gpgpuSupport) {
			if (!session.cleanOutput) {
				warnUser(
					"Hardware acceleration isn't detected.<br /><br />This effect will not work",
					4000
				);
				return;
			}
			...
			if (!session.cleanOutput) {
				warnUser(
					"Hardware acceleration isn't detected.<br /><br />Please enable it for this effect to work correctly.<br /><br /><i>Settings -> Advanced -> System -> Use hardware-accleration</i>"
				);
			}
			return;
		}
...
function loadEffect(effect) {
	...
	script.onload = function () {
		...
		if (gpgpuSupport == "Google SwiftShader") {
			if (!session.cleanOutput) {
				warnUser(
					"Hardware acceleration isn't detected.<br /><br />Please enable it for better performance.<br /><br /><i>Settings -> Advanced -> System -> Use hardware-accleration</i>"
				);
			}
		}
	};
	script.src =
		"./filters/" + filename + ".js?" + parseInt(1000 * Math.random());
	document.head.appendChild(script);
...
function loadTensorflowJS() {
	...
	script4.onload = function () {
		async function loadModel() {
			session.TFJSModel = await faceLandmarksDetection.load(
...
async function loadTFLiteModel() {
	...
	session.tfliteModule.HEAPU8.set(
		new Uint8Array(session.tfliteModule.model),
		session.tfliteModule._getModelBufferMemoryOffset()
	);
...
function getDetailedState() {
		...
		if (session.rpcs[UUID].streamID) {
			...
			try {
				if (document.getElementById("guestFeeds")) {
					var lock = parseInt(
						document.getElementById("position_" + UUID).dataset.locked
					);
					if (lock) {
				...
				var scenes = getById("container_" + UUID).querySelectorAll(
					'[data-action-type="addToScene"][data-scene][data--u-u-i-d="' +
						UUID +
						'"]'
				);
				var sceneState = {};
	...
	var scenes = getById("container_director").querySelectorAll(
		'[data-action-type="addToScene"][data-scene]'
	);
	var sceneState = {};
	for (var i = 0; i < scenes.length; i++) {
...
function getGuestTarget(type, id) {
	var element = document.querySelectorAll(
		'[data-action-type="' + type + '"][data-sid="' + id + '"]'
	); // data-sid="P5MQpia"
	if (!element.length) {
		return (element = getRightOrderedElement(
			'[data-action-type="' + type + '"][data--u-u-i-d]',
			id
		));
	} else {
		element = element[0];
...
function getGuestTargetGroup(group, id) {
	var element = document.querySelectorAll(
		'[data-action-type="toggle-group"][data-value="' +
			group +
			'"][data-sid="' +
			id +
			'"]'
...
function oscClient() {
	// OSC (websocket / https API hotkey support).  The iFrame API method provides greater customization.
	if (!session.api) {
		return;
	}
...
function setupCommands() {
	...
	commands.togglehand = function (value) {
		raisehand();
	};
	commands.togglescreenshare = function (value) {
		toggleScreenShare();
	};
	commands.chat = function (value) {
		toggleChat(value);
	};
	...
	commands.hangup = function (value) {
		hangup();
	};
	...
	commands.reload = function (value) {
		location.reload();
	};
	...
	commands.sendChat = function (value) {
		sendChat(value);
	};
	return commands;
}
...
function midiHotkeysCommand_offset(command, value, offset = 1) {
	...
	for (var i = 0; i < 9; i++) {
		if (command == offset + i) {
			var ele = getRightOrderedElement(
				'[data-action-type="mute-guest"][data--u-u-i-d]',
				command - offset
			);
			if (ele) {
				remoteMute(ele, true);
...
function midiHotkeysCommand(command, value) {
	if (command == 110) {
		...
		if (value == 0) {
			var ele = getRightOrderedElement(
				'[data-action-type="forward"][data--u-u-i-d]',
				guestslot
			);
			if (ele) {
			...
			var ele = getRightOrderedElement(
				'[data-action-type="addToScene"][data-scene="1"][data--u-u-i-d]',
				guestslot
			);
			if (ele) {
			...
			var ele = getRightOrderedElement(
				'[data-action-type="mute-scene"][data--u-u-i-d]',
				guestslot
			);
			if (ele) {
				directMute(ele, true);
			...
			var ele = getRightOrderedElement(
				'[data-action-type="hangup"][data--u-u-i-d]',
				guestslot
			);
			if (ele) {
			...
			var ele = getRightOrderedElement(
				'[data-action-type="solo-chat"][data--u-u-i-d]',
				guestslot
			);
			if (ele) {
			...
			var ele = getRightOrderedElement(
				'[data-action-type="toggle-remote-speaker"][data--u-u-i-d]',
				guestslot
			);
			if (ele) {
			...
			var ele = getRightOrderedElement(
				'[data-action-type="toggle-remote-display"][data--u-u-i-d]',
				guestslot
			);
			if (ele) {
			...
			var ele = getRightOrderedElement(
				'[data-action-type="force-keyframe"][data--u-u-i-d]',
				guestslot
			);
			if (ele) {
			...
			var ele = getRightOrderedElement(
				'[data-action-type="addToScene"][data-scene="2"][data--u-u-i-d]',
				guestslot
			);
			if (ele) {
				directEnable(ele, true);
			...
			var ele = getRightOrderedElement(
				'[data-action-type="addToScene"][data-scene="3"][data--u-u-i-d]',
				guestslot
			);
			if (ele) {
				directEnable(ele, true);
			...
			var ele = getRightOrderedElement(
				'[data-action-type="addToScene"][data-scene="4"][data--u-u-i-d]',
				guestslot
			);
			if (ele) {
				directEnable(ele, true);
			...
			var ele = getRightOrderedElement(
				'[data-action-type="addToScene"][data-scene="5"][data--u-u-i-d]',
				guestslot
			);
			if (ele) {
				directEnable(ele, true);
			...
			var ele = getRightOrderedElement(
				'[data-action-type="addToScene"][data-scene="6"][data--u-u-i-d]',
				guestslot
			);
			if (ele) {
				directEnable(ele, true);
			...
			var ele = getRightOrderedElement(
				'[data-action-type="addToScene"][data-scene="7"][data--u-u-i-d]',
				guestslot
			);
			if (ele) {
				directEnable(ele, true);
			...
			var ele = getRightOrderedElement(
				'[data-action-type="addToScene"][data-scene="8"][data--u-u-i-d]',
				guestslot
			);
			if (ele) {
				directEnable(ele, true);
			...
			var ele = getRightOrderedElement(
				'[data-action-type="volume"][data--u-u-i-d]',
				guestslot
			);
			if (ele) {
...
function playbackMIDI(msg, unsafe = false) {
	...
	if (session.midiRemote == 4) {
		...
		session.midiRemote == 1 ||
		session.midiRemote == 2 ||
		session.midiRemote == 3
	) {
		if (msg.d[0] == 156) {
			if (msg.d[1] == 33) {
...
addEventToAll(".column", "click", function (e, ele) {
	...
	if (document.getElementById("empty-container")) {
		getById("empty-container").parentNode.removeChild(
			getById("empty-container")
		);
	}
...
addEventToAll(".close", "click", function (e, ele) {
	...
	var bounding_box =
		getById("empty-container").parentNode.getBoundingClientRect();
	setTimeout(function () {
...
addEventToAll(".column", "animationend", function (e, ele) {
	if (e.animationName == "inlightbox") {
		ele.classList.add("skip-animation2");
		mapToAll(
			".close",
			function (target) {
				target.style.display = "block";
		...
		ele.classList.remove("skip-animation2");
...
addEventToAll(
	"#audioSource",
	"mousedown touchend focusin focusout",
	function (e, ele) {
...
function getReceivers2(UUID) {
	...
	if ("realUUID" in session.rpcs[UUID]) {
		...
		if (!("screenIndexes" in session.rpcs[UUID])) {
			errorlog(
				"this is supposed to be a screen share, but no screen share index was found"
			);
			return;
		}
		ssTracks = session.rpcs[UUID].screenIndexes;
	} else if (
		"screenIndexes" in session.rpcs[UUID] &&
		session.rpcs[UUID].screenIndexes
	) {
		ssTracks = session.rpcs[UUID].screenIndexes;
	}
...
function createSecondStream2(UUID) {
	if (!("allowScreen" in session.pcs[UUID])) {
		return false;
	}
	if ("realUUID" in session.pcs[UUID]) {
		return false;
	...
	if (screenShareState2 == false) {
		...
		navigator.mediaDevices
			...
			.then(function (stream) {
				...
				getById("screenshare3button").title =
					miscTranslations["stop-screen-sharing"];
			});
	} else {
		// removing a screen
		stopSecondScreenshare();
